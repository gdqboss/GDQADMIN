...
class Upload extends Common
{
	...
	public function browser(){
		...
		// 合并其他条件
		if (!empty($where)) {
			$uploadWhere = array_merge($uploadWhere, $where);
		}
		
		// 直接执行查询，显示所有图片
		$count = Db::name('admin_upload')->field($field)->where($uploadWhere)->count();
		$rs = Db::name('admin_upload')->field($field)->where($uploadWhere)->order($sort)->page($pagenum,28)->select()->toArray();
