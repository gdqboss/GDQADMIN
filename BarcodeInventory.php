...
class BarcodeInventory extends Base
{
    ...
    public function outbound()
    {
        ...
        if (Request::isPost()) {
            ...
            Db::name('barcode_inventory_log')->insert([
                'aid' => $aid,
                'bid' => 0,
                'type' => 'outbound_validation_passed',
                'request_data' => json_encode(['specs_parsed' => $specs]),
                'create_time' => time()
            ]);
            ...
            if (!is_array($specs) || empty($specs)) {
                Db::name('barcode_inventory_log')->insert([
                    'aid' => $aid,
                    'bid' => 0,
                    'type' => 'outbound_specs_error',
                    'error_msg' => 'specs is not valid array: ' . gettype($specs),
                    'request_data' => json_encode(['specs' => $specs]),
                    'create_time' => time()
                ]);
            ...
            Db::name('barcode_inventory_log')->insert([
                'aid' => $aid,
                'bid' => 0,
                'type' => 'outbound_get_product_start',
                'request_data' => json_encode(['proid' => $proid]),
                'create_time' => time()
            ]);
            ...
            Db::name('barcode_inventory_log')->insert([
                'aid' => $aid,
                'bid' => 0,
                'type' => 'outbound_get_product_success',
                'request_data' => json_encode(['product_id' => $product['id'], 'product_name' => $product['name']]),
                'create_time' => time()
            ]);
            ...
            Db::name('barcode_inventory_log')->insert([
                'aid' => $aid,
                'bid' => 0,
                'type' => 'outbound_transaction_start',
                'request_data' => json_encode(['ordernum' => $ordernum]),
                'create_time' => time()
            ]);
            ...
            try {
                Db::name('barcode_inventory_log')->insert([
                    'aid' => $aid,
                    'bid' => 0,
                    'type' => 'outbound_process_specs',
                    'request_data' => json_encode(['specs_count' => count($specs)]),
                    'create_time' => time()
                ]);
                ...
                foreach ($specs as $spec) {
                    ...
                    Db::name('barcode_inventory_log')->insert([
                        'aid' => $aid,
                        'bid' => 0,
                        'type' => 'outbound_process_spec',
                        'request_data' => json_encode(['ggid' => $ggid, 'num' => $num]),
                        'create_time' => time()
                    ]);
                ...
                Db::name('barcode_inventory_log')->insert([
                    'aid' => $aid,
                    'bid' => 0,
                    'type' => 'outbound_transaction_commit',
                    'request_data' => json_encode(['ordernum' => $ordernum]),
                    'create_time' => time()
                ]);
                ...
                Db::name('barcode_inventory_log')->insert([
                    ...
                    'request_data' => json_encode(['ordernum' => $ordernum, 'specs_count' => count($specs)]),
                    'create_time' => time()
                ]);
    ...
    protected function recordStockChange($product, $guige, $num, $ordernum, $type, $price = 0, $outboundType = 0, $dealerId = 0)
    {
        ...
        Db::name('barcode_inventory_log')->insert([
            'aid' => $aid,
            'bid' => 0,
            'type' => 'stock_change_start',
            'request_data' => json_encode(['ordernum' => $ordernum, 'proid' => $product['id'], 'ggid' => $guige['id'], 'num' => $num, 'type' => $type]),
            'create_time' => time()
        ]);
        ...
        try {
            ...
            if (isset($product['bid'])) {
                $minimalLogData['bid'] = $product['bid'];
            }
            
            // 添加备注，将订单号等信息放入备注中，避免依赖其他字段
            ...
            $minimalLogData['remark'] = $remark;
            
            // 尝试插入日志，捕获所有可能的数据库错误
            try {
                // 先尝试直接插入最小化字段集
                Db::name('barcode_stock_log')->insert($minimalLogData);
                // 记录stock_change成功
                Db::name('barcode_inventory_log')->insert([
                    'aid' => $aid,
                    'bid' => 0,
                    'type' => 'stock_change_success',
                    'request_data' => json_encode(['ordernum' => $ordernum, 'proid' => $product['id'], 'ggid' => $guige['id']]),
                    'create_time' => time()
                ]);
            } catch (Exception $e) {
                // 如果插入失败，尝试更简单的字段集，移除可能不存在的字段
                $simplifiedLogData = [
                    'aid' => $aid,
                    'proid' => $product['id'],
                    'ggid' => $guige['id'],
                ...
                try {
                    Db::name('barcode_stock_log')->insert($simplifiedLogData);
                    // 记录stock_change成功
                    Db::name('barcode_inventory_log')->insert([
                        'aid' => $aid,
                        'bid' => 0,
                        'type' => 'stock_change_success',
                        'request_data' => json_encode(['ordernum' => $ordernum, 'proid' => $product['id'], 'ggid' => $guige['id'], 'simplified' => true]),
                        'create_time' => time()
                    ]);
                } catch (Exception $e2) {
                    // 如果还是失败，记录异常但不中断主流程
                    Db::name('barcode_inventory_log')->insert([
                        'aid' => $aid,
                        'bid' => 0,
                        'type' => 'stock_change_exception',
                        'error_msg' => '所有插入尝试均失败: ' . $e->getMessage() . '; ' . $e2->getMessage(),
                        'request_data' => json_encode(['ordernum' => $ordernum, 'proid' => $product['id'], 'ggid' => $guige['id']]),
                        'create_time' => time()
                    ]);
                    // 不抛出异常，避免影响主流程
    ...
    protected function updateStock($proid, $ggid, $num)
    {
        ...
        Db::name('barcode_inventory_log')->insert([
            'aid' => $aid,
            'bid' => 0,
            'type' => 'update_stock_start',
            'request_data' => json_encode(['proid' => $proid, 'ggid' => $ggid, 'num' => $num]),
            'create_time' => time()
        ]);
        ...
        try {
            ...
            Db::name('barcode_inventory_log')->insert([
                ...
                'request_data' => json_encode(['proid' => $proid, 'ggid' => $ggid, 'num' => $num, 'total_stock' => $totalStock]),
                'create_time' => time()
            ]);
