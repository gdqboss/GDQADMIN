<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">信息发布</h1>
        <p class="text-sm text-gray-500 mt-1">公告发布 / 状态管理 / 浏览统计</p>
      </div>
      <button @click="openEdit()" class="px-4 py-2 bg-primary text-white rounded-lg">+ 发布公告</button>
    </div>

    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-4">
      <div class="flex gap-3">
        <input v-model="filter.keyword" placeholder="搜索标题/摘要" class="px-3 py-2 border rounded-lg flex-1" @keyup.enter="search" />
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
      <table class="w-full">
        <thead class="bg-gray-50 text-sm text-gray-600">
          <tr>
            ...
            <th class="px-4 py-3 text-left">置顶</th>
        ...
        <tbody>
          <tr v-for="a in list" :key="a.id" class="border-t hover:bg-gray-50">
            ...
            <td class="px-4 py-3">{{ a.priority ? '⭐ 置顶' : '-' }}</td>
      ...
      <div v-if="!loading && list.length === 0" class="p-8 text-center text-gray-400">暂无公告</div>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑公告' : '发布公告'" width="700px" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px">
        <el-form-item label="标题" required><el-input v-model="form.title" /></el-form-item>
        <el-form-item label="分类">
          <el-select v-model="form.category" class="w-full">
            <el-option label="综合" value="general" /><el-option label="通知" value="notice" />
            <el-option label="活动" value="event" /><el-option label="招募" value="recruit" />
          </el-select>
        </el-form-item>
...
function categoryLabel(c) {
  return { general: '综合', notice: '通知', event: '活动', recruit: '招募' }[c] || c
}
