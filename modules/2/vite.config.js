...
const enabledModules = ["aftersale","ai-classroom","alerts","dashboard","dealers","excel-analyzer","finance","gift-approvals","in-out","job-responsibilities","oa","orders","products","qrcode","referral","reports","retail","returns","roles","settings","stores","suppliers","tasks","transfer","users","warehouses"]

export default defineConfig({
  plugins: [materialSymbolsPreloadPlugin(), vue(), moduleFilterPlugin(enabledModules)],
  resolve: { alias: { '@': resolve(__dirname) } },
  build: {
    outDir: '/root/server/dist-2',
    assetsDir: 'assets',
    sourcemap: false,
    minify: 'esbuild',
    base: '/gdqadmin/',
