...
import { createNativeOrder, getWechatPayConfig, verifyCallback } from '../utils/wechat_pay.js'
import { getServerBaseUrl } from '../utils/server-url.js'
    ...
    try {
      ...
      if (wechatPayConfig) {
        ...
        wechatPay = await createNativeOrder(pool, {
          ...
          notifyUrl: `${baseUrl}/api/pay/wechat/callback`
        })
      }
    } catch (payErr) {
...
// POST /api/pay/create - H5支付下单
router.post('/pay/create', async (req, res, next) => {
  try {
    const { orderNo, amount, description } = req.body

    if (!orderNo || !amount) {
      return res.status(400).json({ code: 400, message: '缺少订单号或金额' })
    }

    const baseUrl = await getServerBaseUrl()
    const wechatPay = await createNativeOrder(pool, {
      orderNo,
    ...
    res.json({ code: 500, message: err.message || '支付下单失败' })
  }
})
...
router.post('/pay/wechat/callback', async (req, res, next) => {
  try {
    ...
    const { sign: _s, ...params } = xml
    const calculatedSign = verifyCallback(params, config.api_key)
    if (!calculatedSign) {
