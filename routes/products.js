...
router.put('/:id', requirePermission(PERMISSIONS.PRODUCTS_WRITE), async (req, res, next) => {
  try {
    const allowedFields = ['name', 'category', 'category_id', 'spec', 'unit', 'supplier', 'purchase_price', 'sale_price', 'safe_stock', 'status', 'image_main', 'require_qrcode', 'group_qr_url', 'group_qr_type']
    const fields = []
    const params = []
    for (const field of allowedFields) {
      if (req.body[field] !== undefined) {
        fields.push(`${field} = ?`)
        // Convert empty strings to null for numeric fields
        if (['purchase_price', 'sale_price', 'safe_stock'].includes(field)) {
