...
// PUT /api/finance-simple/expenses/:id/approve - 审批通过费用支出
router.put('/expenses/:id/approve', async (req, res, next) => {
  ...
  try {
    ...
    if (![ROLES.ADMIN, ROLES.SHOPKEEPER].includes(userRole)) {
      conn.release()
      return res.status(403).json({ code: 403, message: '仅管理员/店长可审批' })
    }

    await conn.beginTransaction()

    const [[expense]] = await conn.query(
      'SELECT id, approval_status, approval_id, amount FROM expense_records WHERE id = ?',
      [expenseId]
    )
    if (!expense) {
      await conn.rollback()
      return res.status(404).json({ code: 404, message: '费用记录不存在' })
    }
    if (expense.approval_status !== 'pending') {
      await conn.rollback()
      return res.status(400).json({ code: 400, message: '该记录无需审批或已审批' })
    }

    // 同步更新 expense_records
    await conn.query(
    ...
    if (expense.approval_id) {
      ...
      if (steps.length === 0) {
        const [[stats]] = await conn.query(
          `SELECT
           ...
           FROM approval_steps WHERE approval_id = ?`,
          [expense.approval_id]
        )
        if (stats.pending_count > 0) {
          await conn.rollback()
          conn.release()
          return res.status(403).json({ code: 403, message: '该单据未分配给您审批' })
        }
      }
      ...
      const [[stepStats]] = await conn.query(
        `SELECT
           SUM(status = 'approved') as approved_count,
      ...
      if (stepStats.rejected_count > 0) {
        approvalStatus = 'rejected'
      } else if (stepStats.approved_count == stepStats.total_count) {
        approvalStatus = 'approved'
      }

      await conn.query(
        `UPDATE approvals SET status = ?, updated_at = IF(? IN ('approved','rejected'), NOW(), updated_at) WHERE id = ?`,
        [approvalStatus, approvalStatus, expense.approval_id]
      )
    }
...
// PUT /api/finance-simple/expenses/:id/reject - 驳回费用支出
router.put('/expenses/:id/reject', async (req, res, next) => {
  const conn = await pool.getConnection()
  try {
    const expenseId = parseInt(req.params.id, 10)
    const userId = req.user.id
    ...
    const [[expense]] = await conn.query(
      'SELECT id, approval_status, approval_id FROM expense_records WHERE id = ?',
      [expenseId]
    )
    if (!expense) {
      await conn.rollback()
    ...
    if (expense.approval_status !== 'pending') {
      await conn.rollback()
      return res.status(400).json({ code: 400, message: '该记录已审批' })
    }

    await conn.query(
      `UPDATE expense_records
       SET approval_status = 'rejected', approver_id = ?, approved_at = NOW()
       WHERE id = ?`,
      [userId, expenseId]
    )
    ...
    if (expense.approval_id) {
      await conn.query(
        `UPDATE approval_steps
         ...
         WHERE approval_id = ? AND approver_id = ? AND status = 'pending'`,
        [reason || '审批驳回', expense.approval_id, userId]
      )

      await conn.query(
        `UPDATE approvals SET status = 'rejected', updated_at = NOW() WHERE id = ?`,
        [expense.approval_id]
      )
    }
...
router.get('/accounts-payable', async (req, res, next) => {
  try {
    ...
    const sql = `
      SELECT ap.supplier_id, s.name as supplier_name, s.phone,
