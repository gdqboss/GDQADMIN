...
// PUT /api/transfer/:id - 编辑调货单（只允许 pending 状态）
router.put('/:id', auth, async (req, res, next) => {
  try {
    const { from_store_id, to_store_id, items, note } = req.body

    const [[existing]] = await pool.query(
      'SELECT status FROM transfer_records WHERE id = ?',
      [req.params.id]
    )
    if (!existing) {
      return res.status(404).json({ code: 404, message: '调货单不存在' })
    ...
    if (existing.status !== 'pending') {
      return res.status(400).json({ code: 400, message: '只能编辑待处理状态的调货单' })
    }
    ...
    try {
      await conn.beginTransaction()
      await conn.query(
        'UPDATE transfer_records SET from_store_id = ?, to_store_id = ?, total_qty = ?, note = ? WHERE id = ?',
        [from_store_id, to_store_id, total_qty, note, req.params.id]
      )
      await conn.query('DELETE FROM transfer_items WHERE record_id = ?', [req.params.id])
      for (const item of items) {
        await conn.query(
          'INSERT INTO transfer_items (record_id, product_id, qrcode_id, quantity) VALUES (?, ?, ?, ?)',
          [req.params.id, item.product_id, item.qrcode_id || null, item.quantity || 1]
        )
      }
      await conn.commit()
      res.json({ code: 0, data: null, message: '调货单更新成功' })
    } catch (err) {
      await conn.rollback()
      throw err
    } finally {
...
// DELETE /api/transfer/:id - 删除调货单（只允许 pending 状态）
router.delete('/:id', auth, async (req, res, next) => {
  try {
    ...
    if (existing.status !== 'pending') {
      return res.status(400).json({ code: 400, message: '只能删除待处理状态的调货单' })
    }
    ...
    try {
      await conn.beginTransaction()
      await conn.query('DELETE FROM transfer_items WHERE record_id = ?', [req.params.id])
      await conn.query('DELETE FROM transfer_records WHERE id = ?', [req.params.id])
      await conn.commit()
      res.json({ code: 0, data: null, message: '调货单删除成功' })
    } catch (err) {
      await conn.rollback()
      throw err
    } finally {
