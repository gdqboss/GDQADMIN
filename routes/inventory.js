    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        ...
        for (const item of items) {
          ...
          // 规则：用 GREATEST 取较大值——不会因为单次入库把已设高的预警值降下来
          if (item.alert_stock && Number(item.alert_stock) > 0) {
            await conn.query(
              'UPDATE products SET alert_stock = GREATEST(COALESCE(alert_stock, 0), ?) WHERE id = ?',
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT ws.id, ws.warehouse_id, ws.product_id, ws.sku_id, ws.quantity, ws.location,
              p.name AS product_name, p.sku AS product_sku, p.image_main,
