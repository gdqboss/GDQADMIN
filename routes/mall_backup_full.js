...
router.get('/products/:id/specs', async (req, res, next) => {
  try {
    ...
    const [specs] = await pool.query(
      'SELECT * FROM product_specs WHERE product_id = ? ORDER BY sort_order', [pid]
    )
    if (specs.length) {
      const sids = specs.map(s => s.id)
      const [values] = await pool.query(
        'SELECT * FROM product_spec_values WHERE spec_id IN (?) ORDER BY sort_order', [sids]
      )
...
router.post('/cart', async (req, res, next) => {
  try {
    ...
    const [[existing]] = await pool.query(`SELECT id, quantity FROM cart_items WHERE ${where}`, params)
    ...
    if (existing) {
      ...
      const [result] = await pool.query(
        'INSERT INTO cart_items (user_id, product_id, sku_id, quantity) VALUES (?, ?, ?, ?)',
...
router.put('/cart/:id', async (req, res, next) => {
  try {
    ...
    const [[item]] = await pool.query('SELECT id FROM cart_items WHERE id = ?', [req.params.id])
...
router.post('/cart/clear', async (req, res, next) => {
  try {
    ...
    await pool.query(`DELETE FROM cart_items WHERE id IN (${placeholders})`, cart_ids)
    res.json({ message: 'ok' })
  } catch (err) { next(err) }
})
...
router.get('/addresses/:id', async (req, res, next) => {
  try {
    const [[row]] = await pool.query('SELECT * FROM addresses WHERE id = ?', [req.params.id])
...
router.post('/addresses', async (req, res, next) => {
  try {
    const { user_id, receiver_name, receiver_phone, province, city, district, address, is_default = false } = req.body
    if (!user_id || !receiver_name || !receiver_phone || !address) {
    ...
    const [result] = await pool.query(
      `INSERT INTO addresses (user_id, receiver_name, receiver_phone, province, city, district, address, is_default)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
router.put('/addresses/:id', async (req, res, next) => {
  try {
    const { receiver_name, receiver_phone, province, city, district, address, is_default } = req.body
    const [[existing]] = await pool.query('SELECT user_id FROM addresses WHERE id = ?', [req.params.id])
    if (!existing) return res.status(404).json({ message: '地址不存在' })

    if (is_default) {
      await pool.query('UPDATE addresses SET is_default = 0 WHERE user_id = ?', [existing.user_id])
    }
    ...
    await pool.query(`UPDATE addresses SET ${fields.join(',')} WHERE id = ?`, vals)
    res.json({ message: 'ok' })
  } catch (err) { next(err) }
})
...
router.put('/addresses/:id/default', async (req, res, next) => {
  try {
    const [[addr]] = await pool.query('SELECT user_id FROM addresses WHERE id = ?', [req.params.id])
    ...
    await pool.query('UPDATE addresses SET is_default = 1 WHERE id = ?', [req.params.id])
...
router.post('/orders', async (req, res, next) => {
  ...
  try {
    ...
    const orderItems = []
    for (const item of items) {
      let price, product_name, product_spec, product_image
      if (item.sku_id) {
      ...
      product_image = prod?.image_main || null
    ...
    const [orderResult] = await conn.query(`
      INSERT INTO orders (order_no, member_id, member_name, member_phone, total_amount,
        freight_amount, discount_amount, pay_amount, receiver_name, receiver_phone,
        receiver_address, status, remark, created_at)
    ...
    for (const item of orderItems) {
      await conn.query(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `, [order_id, item.product_id, item.product_name, item.product_spec, item.product_image, item.price, item.quantity, item.subtotal])
    }

    // 标记优惠券已用
    if (coupon_id) {
      await conn.query(
        'INSERT INTO user_coupons (user_id, coupon_id, order_id, used_at) VALUES (?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE order_id=VALUES(order_id), used_at=VALUES(used_at)',
    ...
    if (from_cart && cart_ids && cart_ids.length) {
      const placeholders = cart_ids.map(() => '?').join(',')
      await conn.query(`DELETE FROM cart_items WHERE id IN (${placeholders})`, cart_ids)
    }
...
router.get('/orders', async (req, res, next) => {
  try {
    ...
    for (const o of rows) {
      const [items] = await pool.query(
        'SELECT product_name, product_spec, product_image, price, number, subtotal FROM order_items WHERE order_id = ?',
        [o.id]
      )
      o.items = items
    }
...
router.put('/orders/:id/pay', async (req, res, next) => {
  try {
    ...
    await pool.query(
      `UPDATE orders SET status='paid', pay_type=?, wechat_trade_no=?, paid_at=NOW() WHERE id=?`,
