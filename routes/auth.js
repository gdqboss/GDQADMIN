...
router.get('/me', async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query('SELECT id, name, email, phone, role, department, supplier_id, status, permissions FROM users WHERE id = ?', [decoded.id])
...
router.post('/login', loginLimiter, async (req, res, next) => {
  try {
    ...
    // 只用手机号查询
    const [rows] = await pool.query('SELECT * FROM users WHERE phone = ?', [loginKey])
    if (!rows.length) {
    ...
    res.json({ code: 0, data: { token, user: userData, permissions } })
  } catch (err) { next(err) }
})
