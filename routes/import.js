...
router.get('/records/:id/summary/by-store/:storeCode', async (req, res) => {
  try {
    ...
    const bySku = skuRows.map(r => {
      const specsRaw = specsMap[r.sku];
      let specs = specsRaw;
      // DB 返回的 specs 是 longtext 字符串（L606 已 '|| "{}"' 兜底），需要 parse
      if (typeof specsRaw === 'string') {
        try { specs = JSON.parse(specsRaw) } catch (e) { specs = {} }
      }
      let color = r.color;
      let size = r.size;
      if (specs && typeof specs === 'object' && !Array.isArray(specs)) {
        // 兼容中英文 key（DB 实际是 "颜色"/"尺寸"，Excel 也常用 "COLOR"/"SIZE"）
        const colorVal = specs['颜色'] || specs['COLOR'] || specs['color'] || specs['Color'];
        const sizeVal  = specs['尺寸'] || specs['SIZE '] || specs['SIZE'] || specs['size'] || specs['Size'];
