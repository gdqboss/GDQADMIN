  ...
  try {
    ...
    const [invStats] = await query(`
      SELECT 
        ...
        SUM(CASE WHEN stock <= 0 THEN 1 WHEN stock < alert_stock THEN 1 ELSE 0 END) as alert_count,
        SUM(CASE WHEN stock > alert_stock THEN 1 ELSE 0 END) as healthy_count
  ...
  try {
    const sql = `
      SELECT 
        ...
        stock, alert_stock, purchase_price, sale_price,
        CASE 
          WHEN stock <= 0 THEN '🔴缺货'
          WHEN stock < alert_stock THEN '🟡低于安全库存'
          ELSE '🟢正常'
        END as alert_level
      FROM products 
      WHERE status = 'active'
        AND (stock <= alert_stock OR stock <= 5)
      ORDER BY stock ASC
    `
  ...
  try {
    const sql = `
      SELECT 
        ...
        p.stock, p.alert_stock, p.purchase_price, p.sale_price,
        COALESCE(s.order_count, 0) as order_count_30d,
        CASE 
          WHEN p.stock <= 0 THEN p.alert_stock * 3
          ELSE GREATEST(0, p.alert_stock * 2 - p.stock)
        END as suggest_quantity,
        CASE 
          WHEN p.stock <= 0 THEN p.alert_stock * 3 * COALESCE(p.purchase_price, 0)
          ELSE GREATEST(0, p.alert_stock * 2 - p.stock) * COALESCE(p.purchase_price, 0)
      ...
      WHERE p.status = 'active' AND p.stock < p.alert_stock * 2
  ...
  try {
    const [stats] = await query(`
      SELECT 
        ...
        SUM(CASE WHEN stock > 0 AND stock < alert_stock THEN 1 ELSE 0 END) as low_stock,
        SUM(CASE WHEN stock >= alert_stock THEN 1 ELSE 0 END) as healthy,
