...
import { buildScanUrl } from '../utils/server-url.js'
...
router.post('/batch', async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 用当前服务器域名生成链接，避免 SGP 生成码指向 claw.gdqshop.cn（北京）
      const scanUrl = await buildScanUrl(code)
      await QRCode.toFile(filePath, scanUrl, { width: 300, margin: 2 })
      codes.push({ code, image_url: `/uploads/qrcodes/${code}.png`, scan_url: scanUrl })
    }
