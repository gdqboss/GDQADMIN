...
const VALID_TRANSITIONS = {
  ...
  sold:        ['activated', 'afterSale', 'inStock', 'disabled'],
...
router.put('/:id/bind', requireRole('admin', 'manager'), async (req, res, next) => {
  try {
    ...
    await pool.query(
      "UPDATE qrcodes SET product_id = ?, sku_id = ?, status = 'bindProduct', bound_at = NOW() WHERE id = ?",
      [product_id, sku_id || null, req.params.id]
    )
...
router.get('/:id/scan-logs', async (req, res, next) => {
  try {
