<template>
  <div class="module-page module-finance-receivable">
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table :data="list" v-loading="loading" style="width: 100%" border @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="paidAmount" label="已收金额" width="130" align="right">
          <template #default="{ row }">
            {{ formatMoney(row.paidAmount) }}
          </template>
        </el-table-column>
        <el-table-column prop="balance" label="待收余额" width="130" align="right">
          <template #default="{ row }">
            <span class="text-danger">{{ formatMoney(row.balance) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" align="center">
          <template #default="{ row }">
            <el-button link type="primary" @click="handleRecord(row)">收款记录</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑应收' : '新增应收'" width="500px">
...
import { getAccountsReceivable } from '@/api/finance'
...
const getStatusText = (status) => {
  const texts = { unpaid: '未收款', partial: '部分收款', paid: '已收款', overdue: '逾期' }
...
const handleAdd = () => {
  isEdit.value = false
  Object.assign(formData, { id: null, customerName: '', orderNo: '', amount: 0, paidAmount: 0, dueDate: '', remark: '' })
