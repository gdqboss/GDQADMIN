...
const ROLE_PAGES = {
  ...
  manager: ['dashboard','bi','excel-analyzer','excel-report-manage','excel-merge-view','reports','products','in-out','warehouses','alerts','qrcode','retail','suppliers','dealers','stores'],
  warehouse: ['dashboard','bi','excel-analyzer','excel-report-manage','excel-merge-view','reports','products','in-out','warehouses','alerts','qrcode','retail'],
}
...
const allNavItems = computed(() => [
  ...
  { key: 'excel-analyzer',       label: t('nav.excelAnalyzer'),       icon: 'analytics',      to: '/excel-analyzer' },
  { key: 'excel-report-manage',  label: t('nav.excelReportManage'), icon: 'folder_open',    to: '/excel-report-manage' },
...
<template>
  <aside class="w-64 bg-sidebar text-white flex flex-col shrink-0 h-screen">
    ...
    <div class="h-14 sm:h-16 flex items-center gap-2 sm:gap-3 px-4 sm:px-6 bg-sidebar-header">
      <img :src="getLogoUrl()" alt="logo" class="w-8 h-8 sm:w-9 sm:h-9 rounded object-cover shadow">
    ...
    <div class="p-3 sm:p-4 border-t border-gray-700">
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <div class="flex flex-col flex-1 min-w-0">
          ...
          <span class="text-[10px] sm:text-xs text-gray-400 truncate">{{ userStore.user?.email || 'admin@gdqshop.cn' }}</span>
        </div>
