...
//        dd($paramdata);
    ...
    public function getStoreProductList()
    {
        ...
        // 查询商品列表
        $productList = Db::name('barcode_inventory')
            ->alias('p')
            ->where($where)
            ->field('p.id, p.name, p.pic')
            ->order($sortOrder)
            ->select()->toArray();
    ...
    public function chooseStoreProduct()
    {
        ...
        $guigeList = Db::name('barcode_inventory_guige')
            ->where('proid', $proid)
            ->where('aid', 1)
            ->field('id, name, stock, barcode, cost_price, sell_price')
            ->order('id asc')
            ->select()->toArray();
        
        if (empty($guigeList)) {
            return json(['status' => 0, 'msg' => '该商品无规格']);
        }
        
        // 如果有多个规格且未指定具体规格，返回商品信息和规格列表让用户选择
        if (count($guigeList) > 1 && empty($ggid)) {
            return json([
                'status' => 2, 
                'msg' => '请选择规格', 
                'product' => $product, 
