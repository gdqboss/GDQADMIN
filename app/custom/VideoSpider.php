...
//        dump($arr);exit;
        if (!empty($arr) && $arr['code']==200){
            //$arr = json_encode($arr,true);
            //小程序对域名有限制，视频先下载到自己服务器
            $url = $arr['data']['url'];
            ...
            $file_name = PRE_URL.'/upload'.$path;

            $this->httpcopy($url,ROOT_PATH.'upload'.$path);
    ...
    public function afterpay($oid){
        $order = Db::name('videospider_order')->where('id',$oid)->find();
        $res = $this->parseUrl($order['url']);
        $data_u = [];
