	...
	public static function initpagecontent($pagecontent,$aid,$mid=-1,$platform='all',$latitude='',$longitude='',$area='',$mendian_id=0,$trid=0,$uid=0){
		...
		if($mid !='-1'){
			if($mid == 0){
				...
				$member_identity = Db::name('member')->where('aid',$aid)->where('id',$mid)->value('identity');
			}
			...
			foreach($pagecontent as $k=>$v){
				...
				if(isset($v['params']['identity']) && !empty($v['params']['identity'])){
					if(!$v['params']['identity']['all'] && !empty($member_identity) && !$v['params']['identity'][$member_identity]){
...
//                                $result[$k2]['luckycollageprolist'] = $luckycollageprolist;
                ...
                if(getcustom('green_score_new',$aid)){
                    //20241112 新增绿色积分修改
                ...
                if(getcustom('yx_cashback_log',$aid)){
                    ...
                    $last_cashback=  Db::name('cashback_log')
                        ...
                        ->where('IFNULL(return_type,"") !=3')
                        ->order('id desc')->value('back_price');
                    $userinfo['last_cashback_price'] = $last_cashback;
                }
