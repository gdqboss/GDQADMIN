...
// +----------------------------------------------------------------------
// | 约车 - 海报 custom_file(car_hailing)
// +----------------------------------------------------------------------
namespace app\controller;
use think\facade\View;
use think\facade\Db;

class CarHailingPoster extends Common
{
    ...
    public function index(){
		...
		$posterset = Db::name('admin_set_poster')->where('aid',aid)->where('type','car_hailing')->where('platform',$type)->order('id')->find();
		...
		if(!$posterset){
			...
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'mp','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'wx','content'=>$data_product_wx]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'alipay','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'baidu','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'toutiao','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'qq','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'h5','content'=>$data_product_mp]);
			Db::name('admin_set_poster')->insert(['aid'=>aid,'type'=>'car_hailing','platform'=>'app','content'=>$data_product_mp]);
	...
	public function save(){
		...
		if(input('post.clearhistory') == 1){
			Db::name('member_poster')->where('aid',aid)->where('type','car_hailing')->where('posterid',$posterset['id'])->delete();
			$msg = '保存成功';
		}else{
			$msg ='保存成功';
		}
		\app\common\System::plog('约车海报设置');
