...
class Business extends Common
{
	...
	public function edit(){
        ...
        if(getcustom('mendian_upgrade')){
            foreach ($menudata as $kk => $vv) {
                if($kk == 'mendian'){
                    unset($menudata[$kk]);
                }
                if($kk == 'shop'){
                   foreach ($vv['child'] as $k => $v) {
                        if($v['path'] == 'MendianOrder/index' || $v['path'] == 'MendianOrder/list'){
                            unset($menudata[$kk]['child'][$k]);
                        }
                    } 
                }
            }
        ...
        if($info['id']){
			...
			if(getcustom('business_agent')){
                ...
                if($uinfo['mid']){
                    ...
                    if($pid != $member['pid'])
                        \app\model\Member::edit(aid,['id'=>$uinfo['mid'], 'pid'=>$pid]);
                }
            }
	...
	public function del(){
        ...
        //约课删除
        if(getcustom('yueke')) {
            Db::name('yueke_category')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('yueke_product')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('yueke_order')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('yueke_worker')->where('aid', aid)->where('bid', 'in', $ids)->delete();
        }
	   //酒店删除
        if(getcustom('hotel')) {
            Db::name('hotel')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('hotel_room')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('hotel_room_prices')->where('aid', aid)->where('bid', 'in', $ids)->delete();
            Db::name('hotel_order')->where('aid', aid)->where('bid', 'in', $ids)->delete();
		    Db::name('hotel_order_yajin')->where('aid', aid)->where('bid', 'in', $ids)->delete();
        }
	...
	public function sysset(){
		if(request()->isPost()){
            ...
            if(!empty($info['wxfw_apiclient_key']) && substr($info['wxfw_apiclient_key'], -4) != '.pem'){
             ...
             if(getcustom('yx_business_miandan')){
				$info['miandan_mianfei_gettj'] = implode(',',$info['miandan_mianfei_gettj']);
			 }
			if($rs){
                //关闭多商户分销设置
		...
		if(getcustom('yx_business_miandan')){
			...
			$info['miandan_mianfei_gettj'] = explode(',',$info['miandan_mianfei_gettj']);
		}
