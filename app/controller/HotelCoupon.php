...
class HotelCoupon extends Common
{
	//列表
	...
	public function edit(){
		...
		if($info && $info['roomids']){
			$hotelroom = Db::name('hotel_room')->where('aid',aid)->where('bid',bid)->where('id','in',$info['roomids'])->order('sort desc,id')->select()->toArray();
		}
		$hotel = array();
		...
		View::assign('hotelroom',$hotelroom);
	...
	public function recordsetst(){
		...
		foreach($rlist as $k=>$v){
			...
			Db::name('coupon_record')->where('aid',aid)->where('bid',bid)->where('id',$v['id'])->update(['status'=>$st,'usetime'=>$usetime]);
