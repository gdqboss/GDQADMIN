...
class ApiYuyueWorker extends ApiCommon
{
	...
	public function initialize(){
		...
		//查看状态
		if(getcustom('yuyue_apply') && $worker){
			if($worker['shstatus']!=1){
				echojson(['status'=>-4,'msg'=>'账号审核未通过','url'=>'/yuyue/yuyue/apply']);
			}
		}
	}
	//服务人员登录
	...
	public function setst(){
		...
		if($st == 3){
			...
			if(getcustom('yuyue_apply') && $order['addmoney']>0 && $order['addmoneyStatus']!=1){
				return $this->json(['status'=>0,'msg'=>'请等顾客支付差价后，再点击完成']);
			}
		...
		if(getcustom('yuyue_apply') && $st==5){  //签到开始服务
			//查看是否在签到范围内
			$set = db('yuyue_set')->where(['aid'=>aid])->find();
			$order = db('yuyue_order')->where(['aid'=>aid,'id'=>$psorder['orderid']])->find();
			$juli = getdistance($this->worker['longitude'],$this->worker['latitude'],$order['longitude'],$order['latitude'],1);
			$juli = round($juli/1000,1);
			if($set['sign_juli']>0 && $juli>$set['sign_juli']){
				return $this->json(['status'=>0,'msg'=>'不在签到范围内']);
			}	
			$updata['status'] = 2;
...
//            }
			...
			if(getcustom('yuyue_apply')){
				$order =  Db::name('yuyue_order')->where('id',$v['orderid'])->find();
				$datalist[$key]['addprice'] = $order['addmoney']?$order['addmoney']:0;
				$datalist[$key]['paycode'] = $order['addmoneyPaycode']?$order['addmoneyPaycode']:'';
				$datalist[$key]['addprice'] = $order['addmoney']?$order['addmoney']:0;
				$datalist[$key]['addmoneystatus'] = $order['addmoneyStatus'];
				$datalist[$key]['addmoneyPayorderid'] = $order['addmoneyPayorderid']?$order['addmoneyPayorderid']:0;
			}
