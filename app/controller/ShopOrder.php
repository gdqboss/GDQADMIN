...
class ShopOrder extends Common
{
    ...
    public function iswxshipping(){
        $rs = \app\common\Wechat::isTradeManaged($this->aid);
        dump($rs);
        $rs = \app\common\Wechat::get_delivery_list($this->aid);
        dump($rs);
        $rs2 = \app\common\Order::getWxExpressCompany($this->aid,'申通快递');
        dd($rs2);
    }
    //重新触发微信发货
    public function rWxshipping(){
        ...
        if($this->bid == 0){
            $order = Db::name($type.'_order')->where('id',$orderid)->where('aid',$this->aid)->find();
        }else{
            $order = Db::name($type.'_order')->where('id',$orderid)->where('aid',$this->aid)->where('bid',$this->bid)->find();
        }
        if(!$order) return json(['status'=>0,'msg'=>'订单不存在']);
        dump($order);

        //发货信息录入 微信小程序+微信支付
        $rs = \app\common\Order::wxShipping($this->aid,$order,$type);
        dd($rs);
    }
