...
class ApiShop extends ApiCommon{
	...
	public function getprolist(){
		...
		// 保存原始member对象，用于后续恢复
		$originalMember = $this->member;
		// 对于 classify.vue 页面，临时将member设为null，强制使用默认级别
		$this->member = null;
	...
	public function classify(){
		if(input('param.bid')){
            ...
            // 对于 classify.vue 页面，始终使用默认级别，不考虑会员等级
            $where2 = "find_in_set('-1',showtj)";
            $tjwhere[] = Db::raw($where2);
			$clist = Db::name('shop_category')->where($tjwhere)->where('aid',aid)->where('pid',0)->where('status',1)->order('sort desc,id')->select()->toArray();
//			dd($clist);
	...
	public function product(){
        ...
        if(getcustom('product_pickup_device')){
            ...
            $device_no = $goods_lane = $dgid = '';
            if($devicedata){
                $devicedata_arr = explode(',',$devicedata);
                $device_no = isset($devicedata_arr[0]) ? $devicedata_arr[0] : '';
                $goods_lane = isset($devicedata_arr[1]) ? $devicedata_arr[1] : '';
                $dgid = isset($devicedata_arr[2]) ? $devicedata_arr[2] : '';
            }
            if($goods_lane && $device_no){
                ...
                if($device_goods && $device_goods['real_stock'] ==0 && $type ==0 && !$dgid){
                ...
                if($device_goods && $device_goods['real_stock'] ==0){
        ...
        if(getcustom('plug_tengrui')) {
            ...
            $tr_roomId = isset($check_product['tr_roomId']) ? $check_product['tr_roomId'] : '';
        }
        ...
        if($tjdatalist){
            foreach($tjdatalist as &$tv){
                ...
                $gglist = Db::name('shop_guige')->where('proid',$tv['id'])->select()->toArray();
                foreach($gglist as $k=>$v){
                    if(getcustom('member_level_price_show')){
                        //获取第一个规格的会员等级价格
...
//		    $updata['createtime'] = time();
	...
	public function createOrder(){
		...
		foreach($buydata as $data){
			...
			// 添加supplier_id - 优先使用商品的supplier_id
			...
			foreach($prolist as $key=>$v){
				...
				// 添加supplier_id - 优先使用商品的supplier_id
				$ogdata['supplier_id'] = intval($product['supplier_id'] ?? 0);
                if(getcustom('product_supply_chain')){
