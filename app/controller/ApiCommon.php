...
class ApiCommon extends ApiBase
{
    ...
    public function initialize(){
        ...
        if(getcustom('fengkuangkanjia') && $this->member && !in_array(request()->controller(),['ApiAddress','ApiPay','ApiImageupload','ApiKanjia']) && !in_array(request()->controller().'/'.request()->action(),['ApiKanjia/product','ApiKanjia/join','ApiKanjia/helplist','ApiKanjia/orderlist','ApiKanjia/orderdetail','ApiKanjia/buy','ApiKanjia/createOrder','ApiKanjia/product','ApiIndex/getCustom','ApiShop/mendian'])){
			$kanjiaset = Db::name('kanjia_sysset')->where('aid',aid)->find();
			if($kanjiaset['mastpay'] == 1){
				$joininfo = Db::name('kanjia_join')->where('aid',aid)->where('mid',mid)->where('status',1)->where('isbuy',0)->order('id desc')->find();
				if($joininfo){
					$product = Db::name('kanjia_product')->where('aid',aid)->where('status',1)->where('ischecked',1)->where('id',$joininfo['proid'])->find();
					if($product){
						echojson(['status'=>-4,'msg'=>'您有砍价完成的订单待支付,请前往支付','url'=>'/activity/kanjia/join?proid='.$joininfo['proid']]);
					}
				}else{
					$kjorder = Db::name('kanjia_order')->where('aid',aid)->where('mid',mid)->where('status',0)->order('id desc')->find();
					if($kjorder){
						$product = Db::name('kanjia_product')->where('aid',aid)->where('status',1)->where('ischecked',1)->where('id',$kjorder['proid'])->find();
						if($product){
							echojson(['status'=>-4,'msg'=>'您有砍价完成的订单待支付,请前往支付','url'=>'/activity/kanjia/join?proid='.$kjorder['proid']]);
						}
					}
				}
			}
	...
	public function _getposter($aid,$bid,$platform,$posterdata,$page,$scene,$textReplaceArr){
		...
		foreach ($poster_data as $d){
			...
			if ($d['type'] == 'qrwx' || $d['type'] == 'qrwxreg'){
				...
				$res = imagettftext($target, $d['size'], 0, $d['left'], $d['top'] + $d['size'], $color, $font,  $d['content']);
        ...
        foreach($datalist as $k=>$v){
            if($this->member) {
                ...
                if (getcustom('member_product_price')) {
                    //一客一价,存在用户时，存在设置的专享商品时 ,只查询商品 不查询规格的 默认售价升序 一条
                    $member_product = Db::name('member_product')->where('aid', aid)->where('mid', mid)->where('proid', $v['id'])->order('sell_price asc')->find();
                    if ($member_product) {
                        $datalist[$k]['sell_price'] = $member_product['sell_price'];
                        $datalist[$k]['givescore'] = $member_product['givescore'];
                    }
                }
            }
            $fwnames = [];
