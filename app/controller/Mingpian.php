...
class Mingpian extends Common
{
    ...
    public function set(){
		...
		if(getcustom('mingpian_addfields')){
			...
			$addfieldnum = 0;
			if($info['field_list']){
				$fieldnum = count($info['field_list'])-1;
				$addfieldnum = $info['field_list'][$fieldnum]??0;
			}
			View::assign('addfieldnum',$addfieldnum);
			View::assign('defaulticon',PRE_URL.'/static/img/mingpian/default.png');
		}
		return View::fetch();
    }
	//删除
