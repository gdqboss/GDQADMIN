...
//		if(bid != 0 && $order['bid']!=bid) showmsg('无权限操作');
    ...
    public function getJindu($express_no){
        ...
        if($order['fwtype']==1){
            ...
            if(getcustom('yuyue_apply')){
                if($psorderinfo['starttime']){
                    $list[] = ['time'=>date('Y-m-d H:i',$psorderinfo['starttime']),'context'=>'服务人员'.$psuser['realname'].'('.$psuser['tel'].')'.'已接单'];
                }
                if($psorderinfo['daodiantime']){
                    $list[] = ['time'=>date('Y-m-d H:i',$psorderinfo['daodiantime']),'context'=>'服务人员已出发'];
                }
                if($psorderinfo['sign_time']){
                    $list[] = ['time'=>date('Y-m-d H:i',$psorderinfo['sign_time']),'context'=>'服务人员已开始服务'];
                }
            }else{
                if($psorderinfo['starttime']){
	...
	public function collect(){ //确认完成
		...
		if(getcustom('yuyue_apply') && $order['addmoney']>0 && $order['addmoneyStatus']!=1){
			return json(['status'=>0,'msg'=>'请等顾客支付差价后，再点击完成']);
		}
		$updata = [];
		$updata['status'] = 3;
		$updata['endtime'] = time();
