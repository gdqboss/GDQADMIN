...
use think\facade\Db;
class DailyjinjuGanwu extends Common
{
    ...
    public function index(){
		if(request()->isAjax()){
			...
			$count = 0 + Db::name('dailyjinju_ganwu_order')->where($where)->count();
			$list = Db::name('dailyjinju_ganwu_order')->where($where)->page($page,$limit)->order($order)->select()->toArray();
	...
	public function excel(){
		...
		$list = Db::name('dailyjinju_ganwu_order')->where($where)->order($order)->select()->toArray();
	...
	public function del(){
        $ids = input('param.ids');
        Db::name('dailyjinju_ganwu_order')->where('id','in',$ids)->where('aid',aid)->where('bid',bid)->delete();
	...
	public function setst(){
		...
		Db::name('dailyjinju_ganwu_order')->where($where)->update(['is_checked'=>$st]);
		\app\common\System::plog('每日金句感悟改状态'.implode(',',$ids));
