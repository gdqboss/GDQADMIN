	...
	public function index(){
		...
		if(getcustom('plug_businessqr') && $this->member['bid']){
			...
			if($needgoto){
                ...
                if(!$pagedata){
                    return $this->json(['status'=>-3,'url'=>'/pages/shop/fastbuy2?bid='.$this->member['bid']]);
                }else{
                    return $this->json(['status'=>-3,'url'=>'/pages/business/index?id='.$this->member['bid']]);
                }
			}
		}
				...
				if($fromid){
                    ...
                    if($sysset['reg_invite_code']) {
                        ...
                        if($sysset['reg_invite_code_type'] == 1) {//邀请码
                            ...
                            $upuser = Db::name('member')->where('aid',aid)->where('id',intval($yqcode))->find();
                        } else {//手机号
			...
			if($rs['openid']){
				...
				if(!$member){
					...
					if(!$member['headimg'] || strpos($member['headimg'],'/static/img/touxiang.png') !== false){
						...
						Db::name('member')->where('aid',aid)->where('id',$member['id'])->update($update);
					}
                    //后绑定开放平台有了unionid 同步更新
                    if(empty($member['unionid']) && $result['unionid']){
                        Db::name('member')->where('aid',aid)->where('id',$member['id'])->update(['unionid'=>$result['unionid']]);
	...
	public function wxlogin(){
		...
		if(!$member){
            ...
            if($authlogin!=2){
                ...
                // 检查是否来自港货超市登录页面
                $frompage = input('param.frompage', '');
                if($frompage == '/pages/index/hklogin' || ($this->sysset['login_setnickname'] && (!$userinfo['nickName'] || $userinfo['nickName'] == '微信用户' || !$userinfo['avatarUrl'] || strpos($userinfo['avatarUrl'], '/static/img/touxiang.png') !== false))){
			...
			// 检查是否来自港货超市登录页面
			$frompage = input('param.frompage', '');
			if($authlogin!=2 && ($frompage == '/pages/index/hklogin' || ($this->sysset['login_setnickname'] && ($member['nickname'] == '微信用户' || $member['nickname'] == '关注用户' || !$member['headimg'] || strpos($member['headimg'], '/static/img/touxiang.png') !== false)))){
				$rs['userinfo'] = $userinfo;
				cache($this->sessionid.'_'.platform.'UserInfo',$rs,3600);
                return $this->json(['status'=>3,'msg'=>'设置头像昵称','login_setnickname'=>$this->sysset['login_setnickname'],'login_bind'=>$this->sysset['login_bind']]);
            }
...
//			}
    ...
    public function wxRegister(){
        ...
        if(!$member){
            ...
            if(strpos($fromPage, 'hklogin') !== false || strpos($requestUri, 'hklogin') !== false){
                $fromHkLogin = true;
            }
            
            // 对于港货超市入口的用户，强制检查头像和昵称
            $requiredHeadImg = $fromHkLogin || $this->sysset['login_setnickname'];
            $requiredNickname = $fromHkLogin || $this->sysset['login_setnickname'];
            
            // 获取用户头像和昵称
            $headimg = input('param.headimg') ? input('param.headimg') : ($userinfo['avatarUrl'] ? \app\common\Pic::uploadoss($userinfo['avatarUrl']) : '');
            $nickname = input('param.nickname') ? input('param.nickname') : $userinfo['nickName'];
            
            // 检查头像
            if($requiredHeadImg && (!$headimg || strpos($headimg, '/static/img/touxiang.png') !== false)){
                return $this->json(['status'=>3,'msg'=>'请设置真实头像','login_setnickname'=>true]);
            }
            
            // 检查昵称
            if($requiredNickname && (!$nickname || $nickname == '微信用户')){
                return $this->json(['status'=>3,'msg'=>'请设置真实昵称','login_setnickname'=>true]);
            }
...
//                    }
    ...
    private function getpid($pid){
        ...
        $upuser = Db::name('member')->where('aid',aid)->where('id',intval($pid))->find();
        if(!$upuser) return $newPid;
        $uplv = Db::name('member_level')->where('aid',aid)->where('id',$upuser['levelid'])->find();
        //等级是否有分销权限
