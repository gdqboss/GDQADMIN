	...
	public function login(){
		...
		if($remember == 1){//自动登录
			...
			if($rs && md5($rs['pwd']) == cookie('password')){
				...
				// 保存语言设置到会话
				$currentLang = input('param.lang', 'zh-cn');
		...
		if(request()->isAjax()){
			...
			if($username=='' || $password==''){
				return json(['status'=>0,'msg'=>t('用户名和密码不能为空')]);
			}elseif($captcha == ''){
				return json(['status'=>0,'msg'=>t('验证码不能为空')]);
			}elseif(!captcha_check($captcha)){
				 return json(['status'=>0,'msg'=>t('验证码错误')]);
			}
			$rs = Db::name('admin_user')->where('un',$username)->where('pwd',md5($password))->find();
			if(!$rs){
				return json(['status'=>2,'msg'=>t('用户名或密码错误')]);
			}elseif($rs['status']!=1){
				return json(['status'=>0,'msg'=>t('该账号已禁用')]);
			}
			if($rs['bid'] > 0){
				$binfo = Db::name('business')->where('id',$rs['bid'])->find();
				if($binfo['status'] != 1){
					if($binfo['status'] == -1)
						return json(['status'=>0,'msg'=>t('已过期，请续费')]);
					else
						return json(['status'=>0,'msg'=>t('该商家尚未审核通过')]);
				}
			}
			...
			if(input('param.fromurl')){
				return json(['status'=>1,'msg'=>t('登录成功'),'url'=>input('param.fromurl')]);
			}else{
				return json(['status'=>1,'msg'=>t('登录成功'),'url'=>(string)url('Backstage/index')]);
			}
		}
