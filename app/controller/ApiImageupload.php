...
class ApiImageupload extends ApiCommon{
	...
	public function uploadbase64(){
		...
		if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
			...
			if(!in_array($type,explode(',',config('app.upload_type')))){
				return '';
			}
	...
	function uploadImg(){
		...
		if($file){
			try {
			    $originalExt = strtolower($file->getOriginalExtension());
                $rinfo['extension'] = $originalExt ? $originalExt : 'png';
                
                // 只验证文件扩展名，不验证MIME类型，避免因MIME类型不匹配导致上传失败
