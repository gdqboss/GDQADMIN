...
// +----------------------------------------------------------------------
// | 智能开门 - 记录 custom_file(lot_cerberuse)
// +----------------------------------------------------------------------
namespace app\controller;
use think\facade\View;
use think\facade\Db;
class CerberuseOrder extends Common
{
	//订单列表
    public function index(){
		...
		if(request()->isAjax()){
			...
			if(input('param.proname')) $where[] = ['title','like','%'.input('param.proname').'%'];
            ...
            if(input('param.zctime') ){
                ...
                $where[] = ['starttime','>=',strtotime($zctime[0])];
                $where[] = ['endtime','<',strtotime($zctime[1])];;
            }
			...
			$count = 0 + Db::name('cerberuse_order')->where($where)->count();
			$list = Db::name('cerberuse_order')->where($where)->page($page,$limit)->order($order)->select()->toArray();
			foreach($list as $k=>$vo){
				$member = Db::name('member')->where('id',$vo['mid'])->find();
				$cerberuse =Db::name('cerberuse')->where('aid',aid)->where('id',$vo['proid'])->find();
				$list[$k]['goodsdata'] = '<div style="font-size:12px;float:left;clear:both;margin:1px 0">'.
					'<img src="'.$cerberuse['pic'].'" style="max-width:60px;float:left">'.
					...
					'<div style="padding-top:0px;color:#f60;font-size:12px; font-weight: 700">￥'.$vo['product_price'].'/小时</div>'.
					'</div>'.
				'</div>';
				$list[$k]['nickname'] = $member['nickname'];
				$list[$k]['headimg'] = $member['headimg'];
				$list[$k]['platform'] = getplatformname($vo['platform']);
                $list[$k]['starttime'] = date('Y-m-d H:i:s',$vo['starttime']);
                $list[$k]['endtime'] = date('Y-m-d H:i:s',$vo['endtime']);
			}
	...
	public function excel(){
		...
		$list = Db::name('cerberuse_order')->where($where)->order($order)->page($page,$limit)->select()->toArray();
        $count = Db::name('cerberuse_order')->where($where)->count();
		$title = array('订单号','下单人','产品名称','单价','时长','实付款','支付方式','客户信息','备注','下单时间','购买时间','订单状态');
			...
			if($vo['status']==0){
             ...
             $edata= [
				...
				date('Y-m-d H:i:s',$vo['createtime']),
                 $vo['starttime'].' 至 '.$vo['endtime'],
				$status,
			];
//            $formdata = \app\model\Freight::getformdata($vo['id'],'cerberuse_order');
//            $formstr = [];
//            if($formdata){
...
//                }
//            }
//            $data[] = array_merge($edata,$formstr);
            $data[] =     $edata;
        }
	...
	public function getdetail(){
		...
		$order = Db::name('cerberuse_order')->where('aid',aid)->where('id',$orderid)->find();
...
//	
//		$formdata = Db::name('freight_formdata')->where('aid',aid)->where('orderid',$orderid)->where('type','cerberuse_order')->find();
//		$data = [];
//		for($i=0;$i<=30;$i++){
//			if($formdata['form'.$i]){
...
//        }
	...
	public function setremark(){
		...
		if(bid == 0){
			Db::name('cerberuse_order')->where('aid',aid)->where('id',$orderid)->update(['remark'=>$content]);
		}else{
			Db::name('cerberuse_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['remark'=>$content]);
		}
	...
	public function changeprice(){
		...
		if(bid == 0){
			Db::name('cerberuse_order')->where('aid',aid)->where('id',$orderid)->update(['totalprice'=>$newprice,'ordernum'=>date('ymdHis').aid.rand(1000,9999)]);
		}else{
			Db::name('cerberuse_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['totalprice'=>$newprice,'ordernum'=>date('ymdHis').aid.rand(1000,9999)]);
		}
	...
	public function closeOrder(){
		$orderid = input('post.orderid/d');
		$order = Db::name('cerberuse_order')->where('id',$orderid)->where('aid',aid)->find();
		if(bid != 0 && $order['bid']!=bid) showmsg('无权限操作');
		...
		if($order['status']==1){
			...
			Db::name('cerberuse_order')->where('id',$orderid)->where('aid',aid)->where('bid',$order['bid'])->update(['status'=>4,'refund_money'=>$order['totalprice'],'refund_status'=>2]);
			//积分抵扣的返还
			if($order['scoredkscore'] > 0){
				\app\common\Member::addscore(aid,$order['mid'],$order['scoredkscore'],'订单退款返还');
			}
			//扣除消费赠送积分
        	\app\common\Member::decscorein(aid,'cerberuse',$order['id'],$order['ordernum'],'订单退款扣除消费赠送');
			//退款成功通知
			$tmplcontent = [];
			...
			$rs = Db::name('cerberuse_order')->where('id',$orderid)->where('aid',aid)->where('bid',bid)->update(['status'=>4]);	
		}
	...
	public function ispay(){
		...
		$orderid = input('post.orderid/d');
		$order = Db::name('cerberuse_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->find();
		Db::name('cerberuse_order')->where('aid',aid)->where('bid',bid)->where('id',$orderid)->update(['status'=>1,'paytime'=>time(),'paytype'=>'后台支付']);
	...
	public function refundCheck(){
		...
		if(bid != 0 && $order['bid']!=bid) showmsg('无权限操作');
		if($st==2){
			Db::name('cerberuse_order')->where('id',$orderid)->where('aid',aid)->update(['refund_status'=>3,'refund_checkremark'=>$remark]);
			//退款申请驳回通知
			$tmplcontent = [];
			...
			Db::name('cerberuse_order')->where('id',$orderid)->where('aid',aid)->update(['status'=>4,'refund_status'=>2]);
	...
	public function del(){
		$id = input('post.id/d');
		if(bid == 0){
			Db::name('cerberuse_order')->where('aid',aid)->where('id',$id)->delete();
		}else{
			Db::name('cerberuse_order')->where('aid',aid)->where('bid',bid)->where('id',$id)->delete();
		}
	...
	public function collect(){ //确认完成
		...
		$updata['status'] = 3;
		$updata['endtime'] = time();
		db('cerberuse_order')->where(['aid'=>aid,'id'=>$orderid])->update(['status'=>3,'collect_time'=>time()]);
		$rs = \app\common\Order::collect($order,'car_hailing');
		if($rs['status'] == 0) return json($rs);
		return json(['status'=>1,'msg'=>'操作成功']);
	}

	public function startSb(){
        $post = input('post.');
        $orderid = intval($post['id']);
        $order = Db::name('cerberuse_order')->where('aid',aid)->where('id',$orderid)->find();
        if(!$order ||  in_array($order['status'],[0,3])){
            return  json( ['status' => 0,'msg' => '订单状态错误']);
        }
        if(time() < $order['starttime'] ){
            return  json( ['status' => 0,'msg' => '订单未开始']);
        }
        if($order['endtime'] <= time()){
            return  json( ['status' => 0,'msg' => '订单已结束']);
        }
        $cerberuse = Db::name('cerberuse')->where('aid',$order['aid'])->where('id',$order['proid'])->find();
        //购买总时长 
        $timeSec = $order['time_length'] * 3600;
        //实际运行时长，当前时长 - 购买时长 
        $yishiyong = time() -  $order['starttime'];
        //实际时长
        $yishiyong = $yishiyong <=0?0:$yishiyong;
        $timeSec = $timeSec -   $yishiyong ;
        if($timeSec > 0){
            ...
            //层级设置 aid/行为/设备id 
            $topic= $order['aid'].'/'.$cerberuse['imei'];
