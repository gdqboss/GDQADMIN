...
//                        Db::name('coupon_record')->where('aid',aid)->where(['mid'=>$orderOrigin['mid'],'id'=>$orderOrigin['coupon_rid']])->update(['status'=>0,'usetime'=>'']);
                    ...
                    if(getcustom('mendian_upgrade')){
                        ...
                        if($admin['mendian_upgrade_status']==1){
                            \app\custom\Mendian::updateCommission($orderOrigin);
                        }
                    }
                    //恢复额度
                    } else {
                ...
                if(getcustom('product_bonus_pool')){
                    foreach ($prolist as $key=>$val){
                        $recordlist = Db::name('member_bonus_pool_record')->where('aid',$val['aid'])->where('ogid',$val['id'])->select()->toArray();

                        foreach ($recordlist as $rkey=>$rval){
                            //修改记录
                            Db::name('member_bonus_pool_record')->where('aid',$rval['aid'])->where('id',$rval['id'])->update(['status' => 2]);
                            //修改奖金池
                            Db::name('bonus_pool')->where('id',$rval['bpid'])->where('aid',$rval['aid'])->update(['status' => 0,'mid' => 0]);
                        }
                    }
                }
                //erp旺店通退款
