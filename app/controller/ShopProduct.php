...
//        dd($paramdata);
	...
	// 显示选择仓库商品弹窗
    public function showChooseStoreProduct()
    {
        View::assign('title', '选择仓库商品');
        return View::fetch('shop_product/choose_store_product');
    }
    ...
    public function getStoreProductList()
    {
        ...
        // 联合查询，获取完整的商品信息
        $guigeList = Db::name('barcode_inventory_guige')
            ->alias('g')
            ->join('barcode_inventory p', 'g.proid = p.id', 'left')
            ->where($where)
            ->field('g.*, p.name as product_name, p.pic as product_pic')
            ->order('g.id desc')
            ->select()->toArray();
        
        // 处理数据格式，适应前端需要
        $formattedList = [];
        foreach ($guigeList as $guige) {
            $formattedList[] = [
                'id' => $guige['proid'],
                'name' => $guige['product_name'] ?? $guige['name'],
                'stock' => $guige['stock'],
                'guige' => [
                    [
                        'id' => $guige['id'],
                        'name' => $guige['name'],
                        'stock' => $guige['stock'],
                        'barcode' => $guige['barcode'] ?? '',
                        'cost_price' => $guige['cost_price'] ?? 0,
                        'sell_price' => $guige['sell_price'] ?? 0
                    ]
                ]
            ];
        }
    ...
    // 选择仓库商品
    public function chooseStoreProduct()
    {
        ...
        if (!$guige) {
            return json(['status' => 0, 'msg' => '规格不存在']);
        }
        ...
        $product = [
            ...
            'name' => $guige['product_name'] ?? $guige['name'],
            'stock' => $guige['stock'],
            'pic' => $guige['product_pic']
        ];
        
        return json(['status' => 1, 'msg' => '选择成功', 'product' => $product, 'guige' => $guige]);
    }
    ...
    public function updateStoreStock()
    {
        ...
        if (empty($proid) || empty($ggid) || $num <= 0) {
            return json(['status' => 0, 'msg' => '参数错误']);
        }
        
        // 获取aid，与BarcodeInventory控制器保持一致，使用固定值1
        $aid = 1;
        
        // 开启事务
        Db::startTrans();
        try {
            // 直接检查规格是否存在，因为没有proid关联字段
            $guige = Db::name('barcode_inventory_guige')
                ->where('id', $ggid)
                ->where('aid', $aid)
                ->find();
            ...
            // 更新规格库存 - 使用框架自动添加前缀，添加aid条件
            $result = Db::name('barcode_inventory_guige')
                ->where('id', $ggid)
                ->where('aid', $aid)
                ->dec('stock', $num)
                ->update();
            ...
            Db::name('barcode_inventory_log')->insert([
                'aid' => $aid,
                'bid' => bid,
                'proid' => $ggid, // 使用规格id作为proid，因为没有真正的proid
                'ggid' => $ggid,
                'num' => -$num,
                'type' => 'out',
                'ordernum' => $ordernum,
            ...
            return json(['status' => 1, 'msg' => '出库成功']);
        } catch (Exception $e) {
            // 回滚事务
            Db::rollback();
            return json(['status' => 0, 'msg' => '出库失败：' . $e->getMessage()]);
        }
    }
}
