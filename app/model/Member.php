...
class Member extends Model
{
	...
	function add($aid,$data){
        ...
        if(getcustom('member_overdraft_money')){
            ...
            if($admin['overdraft_init']>0){
                $data['overdraft_money'] = $admin['overdraft_init'];
            }
        }
...
//			self::updatepath_bak($allpath);
    ...
    function updatepath($pid,$new_path=''){
        ...
        $memberlist = Db::name('member')->where('aid',aid)->where("find_in_set({$pid},path)")->field('id,pid,path')->order('id')->select()->toArray();
        ...
        foreach($memberlist as $member){
            ...
            $path = ltrim($path,',');
            Db::name('member')->where('aid',aid)->where('id',$member['id'])->update(['path'=>$path]);
