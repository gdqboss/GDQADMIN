				...
				<view class="order-box" @tap="toggleOrderExpand(item.id)">
					...
					<view class="order-content-expand" v-if="expandedOrders[item.id]">
						...
						<view class="order-info">
							...
							<!-- 供货商ID，管理员可编辑 -->
							<view class="supplier-id">
								<span>供货商ID：</span>
								<template v-if="isSystemAdmin">
									<picker mode="selector" :range="suppliers" :range-key="'name'" :value="getOrderSupplierIndex(item.supplier_id)" @change="changeOrderSupplier($event, item)">
										<view class="editable-supplier-id">
											{{ getOrderSupplierName(item.supplier_id) }}
											<text class="iconfont iconjiantou" style="color:#999;font-weight:normal;"></text>
										</view>
									</picker>
								</template>
								<template v-else>
									{{item.supplier_id && item.supplier_id !== 0 ? item.supplier_id : '未指定'}}
								</template>
							</view>
						</view>
...
export default {
  ...
  onLoad: function () {
        ...
        const getUserInfo = () => {
            ...
            try {
                ...
                if (cachedUserInfo && typeof cachedUserInfo === 'object' && Object.keys(cachedUserInfo).length > 0) {
  ...
  methods: {
    ...
    checkPermission: function () {
        ...
        // 确保this.userInfo是一个对象
        if (!this.userInfo || typeof this.userInfo !== 'object') {
            this.userInfo = {};
        }
        
        // 直接从userInfo获取字段，不再尝试深拷贝
        // 后端已经返回了正确的userinfo数据
        ...
        const memberLevel = this.userInfo.member_level;
        
        // 获取会员名称，可能在memberLevelName字段中
        const memberLevelName = this.userInfo.member_level_name || this.userInfo.level_name || '';
        
        console.log('memberLevel:', memberLevel, 'memberLevelName:', memberLevelName, 'userType:', userType, 'type:', type, 'id:', id);
        
        // 系统管理员判断 - 简化条件，仅需要memberLevel == 9
        this.isSystemAdmin = memberLevel == 9;
        
        console.log('isSystemAdmin:', this.isSystemAdmin);
        
        // 系统管理员直接允许访问，不需要检查其他权限
        if (this.isSystemAdmin) {
            console.log('系统管理员或智慧家长，直接允许访问');
            this.hasSupplierPermission = true;
            this.supplierId = -1; // 系统管理员可以查看所有供货商的订单
            return;
        }
        
        // 非系统管理员只需要满足：关联了供货商ID
        console.log('supplierId:', supplierId);
        
        // 只有关联了供货商ID才能访问
        this.hasSupplierPermission = supplierId > 0;
        ...
        // 店主/供货商/仓管，设置固定的supplierId，只能查看自己关联的供货商订单
        // 只有当supplierId大于0时才设置，否则保持原来的值
        if (supplierId > 0) {
            this.supplierId = supplierId;
        }
        console.log('非系统管理员，设置supplierId为:', this.supplierId);
    },
    // 获取订单列表
    getdata: function (loadmore) {
      ...
      app.post('ApiAdminOrder/weightOrderFahuoList', {
        ...
        if (res && res.status == 1) {
          ...
          // 处理后端返回的userinfo，保存到globalData和缓存
        if (res.userinfo) {
          console.log('API返回的用户信息:', res.userinfo);
          ...
          try {
            ...
            console.error('存储用户信息到缓存失败:', e);
          }
          
          // 更新当前页面的用户信息
          that.userInfo = res.userinfo;
          
          // 重新检查权限，但不覆盖之前正确设置的supplierId
          // 保存当前的supplierId
          const currentSupplierId = that.supplierId;
          // 重新检查权限
          that.checkPermission();
          // 恢复之前的supplierId（如果有值）
          if (currentSupplierId > 0) {
            that.supplierId = currentSupplierId;
            console.log('恢复之前的supplierId:', that.supplierId);
          }
        }
    ...
    // 获取订单供货商在数组中的索引
    getOrderSupplierIndex: function(supplierId) {
      for (var i = 0; i < this.suppliers.length; i++) {
        if (this.suppliers[i].id === supplierId) {
          return i;
        }
    ...
    // 获取订单供货商名称
    getOrderSupplierName: function(supplierId) {
      for (var i = 0; i < this.suppliers.length; i++) {
        if (this.suppliers[i].id === supplierId) {
          return this.suppliers[i].name;
        }
    ...
    // 修改订单供货商
    changeOrderSupplier: function(e, item) {
      var index = e.detail.value;
      if (index >= 0 && index < this.suppliers.length) {
        var newSupplierId = this.suppliers[index].id;
        console.log('修改订单', item.id, '的供货商ID为:', newSupplierId);
        
        // 调用API来更新订单的供货商ID
        var that = this;
        // 根据订单类型确定type参数
        var orderType = item.order_type == 'collage' ? 'collage' : 'shop';
        
        app.post('ApiAdminOrder/setSupplier', {
          type: orderType,
          orderid: item.id,
          ...
          if (res && res.status == 1) {
            ...
            // 显示错误信息
            app.error(res ? (res.msg || '更新失败') : '更新失败');
          }
        }, function(err) {
          // 显示网络错误
