		...
		<view class="order-content">
			...
			<view class="order-box" @tap.stop="goto" :data-url="'orderdetail?id=' + item.id">
				<view class="head">
					...
					<block v-if="item.status==1">
						<block v-if="item.buytype!=1">
							...
							<text v-if="item.team && item.team.status==2 && item.freight_type!=1  && item.order_pay_type==1" class="st1">拼团成功,待发货</text>
							<text v-if="item.team && item.team.status==2 && item.freight_type==1 && item.order_pay_type==1" class="st1">拼团成功,待提货</text>
															<text v-if="item.team.status==2 && item.order_pay_type==2" class="st1">拼团成功,待发佣金</text>
				...
				<view class="op">
                    ...
                    <block v-if="item.status==3 && (!item.iscomment || item.iscomment==0)">
                        <view class="btn1" :style="{background:t('color1')}" @tap.stop="goto" :data-url="'comment?orderid=' + item.id">我来评价</view>
                    </block>
                    <block v-else-if="item.status==3 && item.iscomment==1">
                        <view class="btn2" @tap.stop="goto" :data-url="'comment?orderid=' + item.id">查看评价</view>
                    </block>
				</view>
			</view>
			</block>
