			...
			<view class="bcontent">
				<view class="product">
					<view class="item flex">
                        <view class="img" @tap="goto" :data-url="'product?id='+product.id"><image class="img" :src="product.pic" mode="aspectFill" lazy-load="true"/></view>
  ...
  methods: {
		getdata: function () {
			...
			app.get('ApiTuangou/buy', {prodata:that.opt.prodata}, function (res) {
				...
				that.weight = weight;
				var leadermoney = 0;
				if (product.leadermoney && product.leadermoney*1 > 0) {
