<?php /*a:4:{s:61:"/www/wwwroot/gdqshop.cn/app/view/designer_category/index.html";i:1747926690;s:48:"/www/wwwroot/gdqshop.cn/app/view/public/css.html";i:1747926690;s:47:"/www/wwwroot/gdqshop.cn/app/view/public/js.html";i:1747926690;s:54:"/www/wwwroot/gdqshop.cn/app/view/public/copyright.html";i:1747926690;}*/ ?>
<!DOCTYPE html><!--custom_file(design_cat)-->
<html>
<head>
  <meta charset="utf-8">
  <title>页面分类</title>
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header">页面分类</div>
          <div class="layui-card-body" pad15>
						<div class="layui-col-md4" style="padding-bottom:10px">
							<a class="layui-btn layuiadmin-btn-list" href="javascript:void(0)" onclick="openmax('<?php echo url('edit'); ?><?php echo $typeparam; ?>')">添加</a>
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" onclick="datadel(0,'<?php echo $type; ?>')">删除</button>
						</div>
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'status', title: '状态',templet:function(d){ return d.status==1?'<span style="color:green">显示</span>':'<span style="color:red">隐藏</span>'}},
      {field: 'operation', title: '操作',templet:function(d){
      	var html = '<div>';
      			<?php if($showtype): ?>
      		  	html += '<button class="table-btn" onclick="openmax(\'<?php echo url('edit'); ?>/id/'+d.id+'<?php echo $typeparam; ?>\')">编辑</button>';
      		  <?php else: ?>
      		  	html += '<button class="table-btn" onclick="openmax(\'<?php echo url('edit'); ?>/id/'+d.id+'&type='+d.type+'&into='+d.into+'\')">编辑</button>';
      		  <?php endif; ?>
      		  html += '<button class="table-btn" onclick="datadel('+d.id+',\'<?php echo $typeparam; ?>\')">删除</button>';
      		  html += "</div>";
      	return html;
      }}
    ]]
	...
	function datadel(id,param=''){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post(go_url,{ids:ids},function(data){
				layer.close(index);
