    ...
    <div class="layui-row layui-col-space15">
      <div class="layui-card layui-col-md12">
				...
				<div class="layui-card-body" pad15>
					...
					<form class="layui-form form-label-w8" id="editForm" action="<?php echo url('save'); ?>" method="post">
							...
							<div class="layui-tab-content">
								<div class="layui-tab-item layui-show">
									<div class="layui-form-item">
										<label class="layui-form-label">拼团推广类型：</label>
										<div class="layui-input-inline" style="width:260px">
											<input type="radio" name="info[promote_type]" value="0" title="普通拼团" <?php if(!$info['promote_type'] || $info['promote_type']==0): ?>checked<?php endif; ?> lay-filter="promote_type"/>
											<input type="radio" name="info[promote_type]" value="1" title="名店推广拼团" <?php if($info['promote_type']==1): ?>checked<?php endif; ?> lay-filter="promote_type"/>
										</div>
									</div>
				...
				<button type="button" class="layui-btn layui-btn-primary" style="margin-left:10px;" onclick="loadLastRecord()">选择上一次记录</button>
									...
									<div class="layui-form-item">
										...
										<div class="layui-input-inline">
											<input type="text" name="info[procode]" class="layui-input" value="<?php echo $info['procode']; ?>">
										</div>
									</div> -->
									<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<select name="info[supplier_id]">
					<option value="" <?php if(!isset($info['supplier_id']) || $info['supplier_id']=='' || $info['supplier_id']===null): ?>selected<?php endif; ?>>--未定供货商--</option>
					<?php foreach($supplierList as $supplier): ?>
						<option value="<?php echo $supplier['id']; ?>" <?php if(isset($info['supplier_id']) && $supplier['id']==$info['supplier_id']): ?>selected<?php endif; ?>><?php echo $supplier['name']; ?></option>
					<?php endforeach; ?>
					</select>
				</div>
				<div class="layui-form-mid layui-word-aux">选择商品关联的供货商</div>
			</div>
									...
									<div class="layui-form-item" id="setgg">
										...
										<div class="layui-input-inline" style="width:auto">
											<table id="ggnamediv"  class="layui-table" style="width:600px">
												...
												<tr>
													<td><input type="text" class="layui-input" name="ggname" value="<?php echo $gg->title; ?>" style="width:120px"/></td>
													<td>
														<?php foreach($gg->items as $k2=>$ggitem): ?>
														<div><input type="text" class="layui-input"style="width:180px" value="<?php echo $ggitem->title; ?>" placeholder="请输入规格名称">
														<?php if($k2==0): ?>
									...
									<?php if(getcustom('collage_givescore_time')): if($bid==0): ?>
										<div class="layui-form-item">
											<label class="layui-form-label">积分赠送时间：</label>
											<div class="layui-input-inline" style="width:500px">
												<input type="radio" name="info[givescore_time]" id="givescore_time" value="0" title="确认收货后" <?php if(!$info['id'] || $info['givescore_time']==0): ?>checked<?php endif; ?> lay-filter=""/>
												<input type="radio" name="info[givescore_time]" id="givescore_time1" value="1" title="拼团成功后" <?php if($info['id'] && $info['givescore_time']==1): ?>checked<?php endif; ?> lay-filter=""/>
											</div>
										</div>
										<?php endif; ?>
									<?php endif; ?>
									<div class="layui-form-item">
										<label class="layui-form-label" id="freight_label">配送模板：</label>
										<div class="layui-input-inline" id="freight_radio_group" style="width:500px">
						...
						<div class="layui-input-inline" style="width:auto">
							<table id="setfreightdiv"  class="layui-table" style="width:500px">
								...
								<?php if($info['freightdata']): $freight_ids = explode(',', $info['freightdata']); foreach($freight_ids as $fid): if($fid): ?>
								<tr class="freightlisttr"><td><?php echo $fid; ?></td><td>配送模板<?php echo $fid; ?></td><td><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="delfreight(this,<?php echo $fid; ?>)">删除</button></td></tr>
								<?php endif; ?>
								<?php endforeach; ?>
								<?php endif; ?>
								<tr id="freightaddtr">
							...
							<input type="hidden" name="info[freightdata]" value="<?php echo $info['freightdata']; ?>"/>
									...
									<div class="layui-form-item" id="promote_store" style="display:none">
						<label class="layui-form-label">选择门店：</label>
						<div class="layui-input-inline" style="width:auto">
							<table id="setmendiandiv" class="layui-table" style="width:500px">
								<thead>
								<tr>
									<th>门店ID</th>
									<th>门店名称</th>
								...
								<?php if($info['mendian_ids']): $mendian_ids = explode(',', $info['mendian_ids']); foreach($mendian_ids as $mid): if($mid): ?>
								<tr class="mendianlisttr"><td><?php echo $mid; ?></td><td>门店<?php echo $mid; ?></td><td><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="delMendian(this,<?php echo $mid; ?>)">删除</button></td></tr>
								<?php endif; ?>
								<?php endforeach; ?>
								<?php endif; ?>
								<tr id="mendianaddtr">
									<td colspan="3"><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="showChooseMendian()">添加</button></td>
								</tr>
							</table>
							<input type="hidden" name="info[mendian_ids]" value="<?php echo $info['mendian_ids']; ?>"/>
						</div>
					</div>
									<script>
									var chooseFreightLayer;
									function showChooseFreight(){
										chooseFreightLayer = layer.open({type:2,title:'选择配送模板',content:"<?php echo url('Freight/choosefreight'); ?>",area:['1000px','600px'],shadeClose:true});
									}
									...
									<?php if((getcustom('collage_limit'))): ?>
									<div class="layui-form-item">
										<label class="layui-form-label">同一个人可以参同一团多次：</label>
										<div class="layui-input-inline">
											<input type="radio" name="info[is_many_times]" value="1" <?php if($info['is_many_times']==1): ?>checked<?php endif; ?> title="是" lay-filter="is_many_times"/>
											<input type="radio" name="info[is_many_times]" value="0" <?php if($info['is_many_times']==0): ?>checked<?php endif; ?> title="否" lay-filter="is_many_times"/>
										</div>
									</div>
									<div class="layui-form-item"  >
										<label class="layui-form-label"><span class="redstar">*</span>每个人每天参团最大次数：</label>
										<div class="layui-input-inline">
											<input type="text" name="info[max_times]" lay-verify="required" lay-verType="tips" class="layui-input" value="<?php echo $info['max_times']; ?>">
										</div>
										<div class="layui-form-mid">次,0不限制</div>
									</div>
									<?php endif; if(getcustom('yx_collage_jieti')): ?>
									<div class="layui-form-item">
										<label class="layui-form-label">拼团类型：</label>
										<div class="layui-input-inline" style="width: 230px">
											<input type="radio" name="info[collage_type]" value="0" <?php if($info['collage_type']==0): ?>checked<?php endif; ?> title="普通拼团" lay-filter="collage_type" />
											<input type="radio" name="info[collage_type]" value="1" <?php if($info['collage_type']==1): ?>checked<?php endif; ?> title="数量拼团" lay-filter="collage_type"/>
										</div>
										<div class="layui-form-mid layui-word-aux" style="margin-left:10px;">普通拼团，根据拼团人数成团，数量拼团，根据拼团结束时间成团</div>
									</div>
								
									<div  id="jietidiv" <?php if(!$info['collage_type']): ?> style="display:none"<?php endif; ?>>
										<div class="layui-form-item">
										...
										<?php  $jietidata = json_decode($info['jieti_data'],true); if($jietidata): foreach($jietidata as $k=>$v): ?>
										<div class="layui-form-item">
											<label class="layui-form-label" ><?php if($k==0): ?>数量拼团设置：<?php endif; ?></label>
											<div class="layui-form-mid">满</div>
											<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[teamnum][]" value="<?php echo $v['teamnum']; ?>" class="layui-input" ></div>
											<div class="layui-form-mid">人，获得</div>
											<div class="layui-input-inline" style="width:100px"><input type="text" name="jt[goodsname][]" value="<?php echo $v['goodsname']; ?>" class="layui-input" ></div>
											<div class="layui-form-mid">和</div>
											<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[goodsnum][]" oninput="getusd2(this,<?php echo $k; ?>)" value="<?php echo $v['goodsnum']; ?>" class="layui-input" ></div>
											<div class="layui-form-mid">课时</div>
	
											<?php if($k==0): ?>
											<button type="button" class="layui-btn layui-btn-primary" onclick="addjtdata()">新增</button>
											<?php else: ?>
										...
										<?php endforeach; else: ?>
										<div class="layui-form-item">
											<label class="layui-form-label" >团购阶梯设置：</label>
	
											<div class="layui-form-mid">满</div>
											<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[teamnum][]" value="" class="layui-input"></div>
											<div class="layui-form-mid">人，获得</div>
											<div class="layui-input-inline" style="width:100px"><input type="text" name="jt[goodsname][]" value="<?php echo $v['goodsname']; ?>" class="layui-input" ></div>
											<div class="layui-form-mid">和</div>
											<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[goodsnum][]" value="" class="layui-input"></div>
											<div class="layui-form-mid">课时</div>
											<button type="button" class="layui-btn layui-btn-primary" onclick="addjtdata()">新增</button>
										</div>
									<?php endif; ?>
										<script>
											function addjtdata(){
												...
												html+='	<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[teamnum][]" value="" class="layui-input" lay-verify="required"></div>';
												html+='	<div class="layui-form-mid">人，获得</div>';
												html+='	<div class="layui-input-inline" style="width:100px"><input type="text" name="jt[goodsname][]" value="" class="layui-input" lay-verify="required"></div>';
												html+='	<div class="layui-form-mid">和</div>';
												html+='	<div class="layui-input-inline" style="width:70px"><input type="text" name="jt[goodsnum][]" value="" class="layui-input" lay-verify="required"></div>';
												html+='	<div class="layui-form-mid">课时</div>';
												html+='	<button type="button" class="layui-btn layui-btn-primary" onclick="deldata(this)">删除</button>';
												html+='</div>';
												$('#jietidiv').append(html);
											}
								...
								<div class="layui-form-item">
									...
									<div class="layui-input-inline">
										<input type="number" name="info[xn_view_num]" value="<?php echo (isset($info['xn_view_num']) && ($info['xn_view_num'] !== '')?$info['xn_view_num']:0); ?>" class="layui-input">
									</div>
									<div class="layui-form-mid layui-word-aux">虚拟浏览数和实际浏览数的和为总浏览数</div>
								</div>
								<div class="layui-form-item">
									<label class="layui-form-label">虚拟分享数：</label>
									<div class="layui-input-inline">
										<input type="number" name="info[xn_share_num]" value="<?php echo (isset($info['xn_share_num']) && ($info['xn_share_num'] !== '')?$info['xn_share_num']:0); ?>" class="layui-input">
									</div>
									<div class="layui-form-mid layui-word-aux">虚拟分享数和实际分享数的和为总分享数</div>
								</div>
								<?php endif; ?>
								<div <?php if($info['collage_type'] && $info['collage_type'] ==1): ?> style="display:none"<?php endif; ?> id="teamnum" >
									<div class="layui-form-item" >
										<label class="layui-form-label">拼团人数：</label>
										<div class="layui-input-inline">
											<input type="text" name="info[teamnum]" value="<?php echo !empty($info['id']) ? $info['teamnum'] : '3'; ?>" class="layui-input">
										</div>
									...
									<div class="layui-form-item">
										...
										<div class="layui-input-inline">
											<input type="text" name="info[buymax]" value="<?php echo !empty($info['id']) ? $info['buymax'] : '0'; ?>" class="layui-input">
										</div>
									...
									<div class="layui-form-item">
										...
										<div class="layui-input-inline">
											<input type="text" name="info[teamhour]" value="<?php echo !empty($info['id']) ? $info['teamhour'] : '36'; ?>" class="layui-input">
										</div>
										<div class="layui-form-mid">小时</div>
										<div class="layui-form-mid layui-word-aux">
											超过该时间人数未满则拼团失败，自动退款</div>
									</div>
								</div>

									<?php if($bid==0): ?>
									<div class="layui-form-item">
										<label class="layui-form-label">团长奖励<?php echo t('积分'); ?>：</label>
										<div class="layui-input-inline">
											<input type="text" name="info[leaderscore]" value="<?php echo !empty($info['id']) ? $info['leaderscore'] : '10'; ?>" class="layui-input">
										</div>
										<div class="layui-form-mid layui-word-aux">拼团成功后赠送团长多少<?php echo t('积分'); ?></div>
									</div>
									<?php endif; if($bid==0): ?>
									<div class="layui-form-item">
							...
							<input type="radio" name="info[commissionset]" value="1" <?php if(!$info['id'] || $info['commissionset']==1): ?>checked<?php endif; ?> title="单独设置提成比例" lay-filter="commissionset">
									...
									if(!$commissiondata3) $commissiondata3 = [];
									 ?>
									<div id="commissionset1" <?php if($info['commissionset']!=1): ?>style="display:none"<?php endif; ?>>
										<?php if(!$aglevellist): ?>
										<div class="layui-form-item">
										...
										<div class="layui-form-item">
											<label class="layui-form-label"  style="width:120px"><?php echo $level['name']; ?>：</label>
											<div class="layui-input-inline layui-module-itemL">
												<div> 一级(%) </div>
												<input type="text" name="commissiondata1[<?php echo $level['id']; ?>][commission1]" class="layui-input" value="<?php echo (isset($commissiondata1[$level['id']]['commission1']) && ($commissiondata1[$level['id']]['commission1'] !== '')?$commissiondata1[$level['id']]['commission1']:2); ?>">
											</div>
											<?php if($level['can_agent']>1): ?>
											<div class="layui-input-inline layui-module-itemL">
												<div>二级(%)</div>
												<input type="text" name="commissiondata1[<?php echo $level['id']; ?>][commission2]" class="layui-input" value="<?php echo (isset($commissiondata1[$level['id']]['commission2']) && ($commissiondata1[$level['id']]['commission2'] !== '')?$commissiondata1[$level['id']]['commission2']:1); ?>">
											</div>
											<?php endif; if($level['can_agent']>2): ?>
											<div class="layui-input-inline layui-module-itemL">
												<div>三级(%)</div>
												<input type="text" name="commissiondata1[<?php echo $level['id']; ?>][commission3]" class="layui-input" value="<?php echo (isset($commissiondata1[$level['id']]['commission3']) && ($commissiondata1[$level['id']]['commission3'] !== '')?$commissiondata1[$level['id']]['commission3']:1); ?>">
											</div>
											<?php endif; ?>
										</div>
										<?php endforeach; ?>
									...
									<div id="commissionset2" <?php if($info['commissionset']!=2): ?>style="display:none"<?php endif; ?>>
										<?php if(!$aglevellist): ?>
										<div class="layui-form-item">
									...
									<div id="commissionset3" <?php if($info['commissionset']!=3): ?>style="display:none"<?php endif; ?>>
										<?php if(!$aglevellist): ?>
										<div class="layui-form-item">
									...
									<?php endif; if($bid==0): if($info['bid']>0): ?>
							<div class="layui-form-item">
                        ...
                        <div class="group_status1 fwtypeExtend" style="clear:both;<?php if($info['group_status']!=1): ?>display:none<?php endif; ?>">
                            ...
                            <div class="layui-input-inline" style="width:auto">
                                <table id="setgroupdiv"  class="layui-table" style="width:500px">
                                    ...
                                    <?php if($info['group_ids']): $group_ids = explode(',', $info['group_ids']); foreach($group_ids as $gid): if($gid): ?>
                                    <tr class="grouplisttr"><td><?php echo $gid; ?></td><td>小区<?php echo $gid; ?></td><td><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="delGroup(this,<?php echo $gid; ?>)">删除</button></td></tr>
                                    <?php endif; ?>
                                    <?php endforeach; ?>
                                    <?php endif; ?>
                                    <tr id="groupaddtr">
                  ...
                  <?php endif; if(getcustom('shop_product_recommend')): ?>
									<div class="layui-form-item">
									...
									<div class="show_recommend2" style="clear:both;<?php if($info['show_recommend']!=2): ?>display:none<?php endif; ?>">
										...
										<div class="layui-input-inline" style="width:auto">
											<table id="setproductdiv"  class="layui-table" style="width:500px">
												...
												<?php if($info['recommend_productids']): $recommend_productids = explode(',', $info['recommend_productids']); foreach($recommend_productids as $pid): if($pid): ?>
												<tr class="productlisttr"><td><?php echo $pid; ?></td><td>商品<?php echo $pid; ?></td><td><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="delProduct(this,<?php echo $pid; ?>)">删除</button></td></tr>
												<?php endif; ?>
												<?php endforeach; ?>
												<?php endif; ?>
												<tr id="productaddtr">
									...
									function showChooseProduct2(){
										chooseProductLayer = layer.open({type:2,title:'选择商品',content:"<?php echo url('CollageProduct/chooseproduct'); ?>/type/2/hidebid/1",area:['1000px','600px'],shadeClose:true});
									}
									...
									<?php endif; if(getcustom('yx_moneypay')): ?>
									<div class="layui-form-item" id="moneypay_section">
										<label class="layui-form-label"><?php echo t('余额'); ?>支付：</label>
										<div class="layui-input-inline">
											<input type="radio" name="info[moneypay]" value="0" title="关闭" <?php if($info['id'] && $info['moneypay']==0): ?>checked<?php endif; ?>/>
											<input type="radio" name="info[moneypay]" value="1" title="开启" <?php if(!$info['id'] || $info['moneypay']==1): ?>checked<?php endif; ?>/>
										</div>
									</div>
									<?php endif; if(getcustom('collage_teampay')): ?>
									<div class="layui-form-item" id="teampay_section">
										<label class="layui-form-label">参团<?php echo t('余额'); ?>支付：</label>
										<div class="layui-input-inline" style="width: 300px">
											<input type="radio" name="info[teampay]" value="0" title="跟随系统" <?php if(!$info['id'] || $info['teampay']==0): ?>checked<?php endif; ?>/>
											<input type="radio" name="info[teampay]" value="1" title="开启" <?php if($info['id'] && $info['teampay']==1): ?>checked<?php endif; ?>/>
											<input type="radio" name="info[teampay]" value="-1" title="关闭" <?php if($info['id'] && $info['teampay']==-1): ?>checked<?php endif; ?>/>
										</div>
										<div class="layui-form-mid layui-word-aux">跟随系统则默认使用拼团系统设置参数；关闭参团余额支付后，只有发起者可用<?php echo t('余额'); ?>支付，其他参团人员不能用</div>
									</div>
									<?php endif; if(getcustom('yx_collage_team_in_team')): if($has_teaminteam): ?>
										<div class="layui-form-item">
											<label class="layui-form-label">团中团设置：</label>
											<div class="layui-input-inline">
												<input type="radio" name="info[teaminteam_status]" value="0" title="关闭" <?php if(!$info['id'] || $info['teaminteam_status']==0): ?>checked<?php endif; ?> lay-filter="teaminteam_status"/>
												<input type="radio" name="info[teaminteam_status]" value="1" title="开启" <?php if($info['id'] && $info['teaminteam_status']==1): ?>checked<?php endif; ?>  lay-filter="teaminteam_status"/>
											</div>
											<div class="layui-form-mid layui-word-aux">团中团分裂新团模式，新团数大于0才起效，参与拼团用户在拼团成功后产生新团，单独购买和发起拼团用户下单成功就产生新团 <?php if($bid>0): ?> <span style="color:red"> 参数的每次变动都需要平台审核</span><?php endif; ?></div>
										</div>
										<div id="setteaminteam" style="<?php if($info['teaminteam_status'] !=1): ?>display: none;<?php endif; ?>">
											<div class="layui-form-item">
												<label class="layui-form-label">团中团新团数：</label>
												<div class="layui-input-inline">
													<input type="text" name="info[teaminteam_splitnum]" value="<?php echo !empty($info['id']) ? $info['teaminteam_splitnum'] : '0'; ?>" class="layui-input">
												</div>
												<div class="layui-form-mid layui-word-aux">
													参与的团拼团成功后会自动分裂N个以用户为团长的新团，大于时0生效
												</div>
											</div>
											<div class="layui-form-item">
												<label class="layui-form-label">团中团提成方式：</label>
												<div class="layui-input-inline" style="width:210px">
													<input type="radio" class="canAgentF-radio" name="info[teaminteam_commissiontype]" value="0" title="百分比" <?php if($info['teaminteam_commissiontype']==0): ?>checked<?php endif; ?>  lay-filter="teaminteam_commissiontype">
													<input type="radio" class="canAgentF-radio" name="info[teaminteam_commissiontype]" value="1" title="固定金额" <?php if($info['teaminteam_commissiontype']==1): ?>checked<?php endif; ?>  lay-filter="teaminteam_commissiontype">
												</div>
												<div class="layui-form-mid layui-word-aux">设置固定金额时按单返<?php echo t('佣金'); ?>，和购买数量无关,成团后立即发放佣金</div>
											</div>
											<div class="layui-form-item">
												<label class="layui-form-label">团中团提成金额：</label>
												<div class="layui-input-inline layui-module-itemL" commission1>
													<div commission1 >一级(<span class="teaminteam_commissionunit"><?php echo $info['teaminteam_commissiontype']==1 ? '元' : '%'; ?></span>) </div>
													<input type="text" name="info[teaminteam_commission1]" class="layui-input" value="<?php echo $info['teaminteam_commission1']; ?>">
												</div>
												<div class="layui-input-inline layui-module-itemL" commission2 >
													<div commission2 >二级(<span class="teaminteam_commissionunit"><?php echo $info['teaminteam_commissiontype']==1 ? '元' : '%'; ?></span>)</div>
													<input type="text" name="info[teaminteam_commission2]" class="layui-input" value="<?php echo $info['teaminteam_commission2']; ?>">
												</div>
												<div class="layui-input-inline layui-module-itemL" commission3 >
														<div commission3>三级(<span class="teaminteam_commissionunit"><?php echo $info['teaminteam_commissiontype']==1 ? '元' : '%'; ?></span>)</div>
													<input type="text" name="info[teaminteam_commission3]" class="layui-input" value="<?php echo $info['teaminteam_commission3']; ?>">
												</div>
											</div>
											<div class="layui-form-item">
												<label class="layui-form-label"></label>
												<div class="layui-input-inline" style="width:80%;color:#999 !important">
														团中团提成分销不以推荐关系为分销，以本团其他成员(不含团长)及新增团其他成员(不含团长)为上下级，拿下级其他成员(不含团长)的提成<br>
														如：有一个团，团长为A，当成员B参与这个团后，这个团拼团成功后会以自动以成员B为团长新增N个新团<br>
														这时，成员B就是团长A的下一级，以成员B为团长新增的那N个团里的其他参与成员(不含团长)，就是团长A的下二级<br>
														然后，若新增的这N个团，有其他成员参与时，拼团成功后也会以其他成员为团长自动分裂的X个团，这X个团里其他参与成员(不含团长)就是团长A下三级，依次类推
												</div>
											</div>
										</div>
										<?php if($info['teaminteam_data'] && $info['teaminteam_data_html']): ?>
											<div class="layui-form-item">
												<label class="layui-form-label" style="color:red">修改待审核：</label>
												<div class="layui-input-inline" style="width:670px">
													<?php echo $info['teaminteam_data_html']; ?>
												</div>
											</div>
											<?php endif; ?>
										<?php endif; ?>
									<?php endif; if(getcustom('yx_queue_free_collage')): if($has_queue_free): ?>
										<div class="layui-form-item">
											<label class="layui-form-label">排队免单设置：</label>
											<div class="layui-input-inline" style="width:300px">
												<input type="radio" name="info[queue_free_status]" value="0" title="跟随系统设置" <?php if(!$info['id'] || $info['queue_free_status']==0): ?>checked<?php endif; ?> lay-filter="queue_free_status">
												<input type="radio" name="info[queue_free_status]" value="1" title="单独设置返利比例" <?php if($info['queue_free_status']==1): ?>checked<?php endif; ?> lay-filter="queue_free_status">
											</div>
											<div class="layui-form-mid layui-word-aux"><?php if($bid>0): ?><span style="color:red">参数的每次变动都需要平台审核</span><?php endif; ?></div>
										</div>
										<div class="layui-form-item" id="queuefreeratebackset" style="<?php if($info['queue_free_status'] != 1): ?>display: none;<?php endif; ?>">
											<label class="layui-form-label">返利比例：</label>
											<div class="layui-input-inline" style="width:150px">
												<input type="number" name="info[queue_free_rate_back]" value="<?php echo $info['queue_free_rate_back']; ?>" class="layui-input">
											</div>
											<div class="layui-form-mid">下单订单给排队订单的返利比例</div>
										</div>
										<?php if($info['queue_free_data'] && $info['queue_free_data_html']): ?>
											<div class="layui-form-item">
												<label class="layui-form-label" style="color:red">修改待审核：</label>
												<div class="layui-input-inline" style="width:670px">
													<?php echo $info['queue_free_data_html']; ?>
												</div>
											</div>
										<?php endif; ?>
										<?php endif; ?>
									...
									<div class="layui-form-item">
										...
										<div class="layui-input-inline">
											<input type="text" name="info[sort]" value="<?php echo !empty($info['id']) ? $info['sort'] : '0'; ?>" class="layui-input">
										</div>
									...
									<div class="layui-form-item">
										...
										<div class="layui-input-inline">
											<input type="radio" name="info[status]" value="1" title="已上架" <?php if(!$info['id'] || $info['status']==1): ?>checked<?php endif; ?>/>
											<input type="radio" name="info[status]" value="0" title="未上架" <?php if($info['id'] && $info['status']==0): ?>checked<?php endif; ?>/>
										</div>
									</div>
...
function geteditordata() {
	...
	<script>
	var chooseMendianLayer;
	function showChooseMendian(){
		chooseMendianLayer = layer.open({type:2,title:'选择门店',content:"<?php echo url('Mendian/choosemendian'); ?>",area:['1000px','600px'],shadeClose:true});
	}
	function choosemendian(res){
		layer.close(chooseMendianLayer);
		var mendian_ids = [];
		var isadd = 0;
		$('.mendianlisttr').each(function(){
			var thisfid = $(this).find('td:eq(0)').html();
			if(thisfid == res.id){
				isadd = 1
				dialog('该门店已添加过了');
			}
			mendian_ids.push(thisfid)
		})
		if(isadd == 0){
			mendian_ids.push(res.id)
			$("input[name='info[mendian_ids]']").val(mendian_ids.join(','));
			$('#mendianaddtr').before('<tr class="mendianlisttr"><td>'+res.id+'</td><td>'+res.name+'</td><td><button type="button" class="layui-btn layui-btn-sm layui-btn-primary" onclick="delMendian(this,'+res.id+')">删除</button></td></tr>');
		}
	}
	function delMendian(obj,fid){
		$(obj).parent().parent().remove();
		var mendian_ids = [];
		$('.mendianlisttr').each(function(){
			mendian_ids.push($(this).find('td:eq(0)').html())
		})
		$("input[name='info[mendian_ids]']").val(mendian_ids.join(','));
	}
		//日期时间选择器
		...
		layui.form.on('radio(is_many_times)', function(data){
			if(data.value == '1'){
				$('#max_times').show();
			}else{
				$('#max_times').hide();
		...
		// 初始化拼团推广类型相关显示状态
		var promoteType = $('input[name="info[promote_type]"]:checked').val();
		if (promoteType == 1) {
			$('#promote_store').show();
		} else {
			$('#promote_store').hide();
		...
		// 初始化配送模板相关显示状态
		freightset($('input[name="info[freighttype]"]:checked').val());
		layui.form.on('radio(promote_type)', function(data){
			if(data.value == '1'){
				$('#promote_store').show();
				$('#freight_label').html('消费地点');
				$('#freight_radio_group').hide();
				$("input[name='info[freighttype]'][value='0']").prop('checked', true);
				// 清空配送模板数据
				$("input[name='info[freightdata]']").val('');
				$('.freightlisttr').remove();
				freightset('0');
				$('#moneypay_section').hide();
				$('#teampay_section').hide();
				$("input[name='info[teamnum]']").val('2');
				$("input[name='info[teamhour]']").val('720');
				setLeaderScoreByFirstPrice();
				setSpecDefaultForPromote();
				$("input[name='info[commissionset]'][value='1']").prop('checked', true);
				$('#commissionset1').show();
				$('#commissionset2').hide();
				$('#commissionset3').hide();
				setCommissionRatesDefault();
				layui.form.render();
			}else{
				$('#promote_store').hide();
				$('#freight_label').html('配送模板');
				$('#freight_radio_group').show();
				$('#moneypay_section').show();
				$('#teampay_section').show();
				$("input[name='info[teamnum]']").val('3');
				$("input[name='info[teamhour]']").val('720');
				setLeaderScoreByFirstPrice();
				layui.form.render();
			}
		})
		function setLeaderScoreByFirstPrice(){
			var v = $("input[name$='[sell_price]']").eq(0).val();
			if(v){
				var score = parseFloat(v)*100;
				$("input[name='info[leaderscore]']").val(score);
			}
		}
		function setSpecDefaultForPromote(){
			$("input[name$='[leader_price]']").eq(0).val('0');
			plset('leader_price');
			$("input[name$='[leader_commission_rate]']").eq(0).val('10');
			plset('leader_commission_rate');
		}
		function setCommissionRatesDefault(){
			$('#commissionset1').find("input[name*='[commission1]']").val('2');
			$('#commissionset1').find("input[name*='[commission2]']").val('1');
			$('#commissionset1').find("input[name*='[commission3]']").val('1');
		}

		<?php if(getcustom('yx_collage_team_in_team')): ?>
		layui.form.on('radio(teaminteam_status)', function(data){
			if(data.value == '1'){
				$('#setteaminteam').show();
			}else{
				$('#setteaminteam').hide();
		...
		layui.form.on('radio(teaminteam_commissiontype)', function(data){
			if(data.value == '1'){
				$('.teaminteam_commissionunit').html('元');
			}else{
				$('.teaminteam_commissionunit').html('%');
			}
		})
		<?php endif; if(getcustom('yx_queue_free_collage')): ?>
		layui.form.on('radio(queue_free_status)', function(data){
			if(data.value == '1'){
				$('#queuefreeratebackset').show();
			}else{
				$('#queuefreeratebackset').hide();
			}
		})
		<?php endif; ?>
		...
		layui.form.on('radio(collage_type)', function(data){
			if(data.value == '1'){
				$('#jietidiv').show();
				$('#teamnum').hide();
			}else{
		...
		function showChooseProduct(){
			layer.open({type:2,title:'选择商品',content:"<?php echo url('ShopProduct/chooseproduct'); ?>/showallbusiness/true",area:['1000px','600px'],shadeClose:true});
		}
		...
		// 同步商品的supplier_id到供货商选择下拉框
		if(data.product.supplier_id) {
			$("select[name='info[supplier_id]']").val(data.product.supplier_id);
			layui.form.render('select');
		}

		<?php if(getcustom('collage_givescore_time')): ?>
			if(data.product.givescore_time == 1){
				$("#givescore_time").removeAttr("checked");
				$("#givescore_time1").prop("checked","true");
			}else{
				$("#givescore_time").prop("checked","true");
				$("#givescore_time1").removeAttr("checked");
			}
			layui.form.render();
		<?php endif; ?>
			//规格
			...
			refreshgg();
				// 填充供应商ID
				$("select[name='info[supplier_id]']").val(data.product.supplier_id || ''); // 注意：使用select选择器，并将默认值改为空字符串
	...
	function loadLastRecord() {
		layer.confirm('确定要加载上一次编辑的记录吗？商品名称不会被覆盖。', {
			btn: ['确定', '取消']
		}, function() {
			// 尝试从localStorage加载上一次记录
			var lastRecord = localStorage.getItem('collage_product_last_record');
			if (lastRecord) {
				lastRecord = JSON.parse(lastRecord);
				
				// 填充表单字段（排除商品名称）
				for (var key in lastRecord) {
					if (key !== 'name') {
						var selector = 'input[name="info[' + key + ']"]';
						var element = $(selector);
						if (element.length > 0) {
							if (element.attr('type') === 'radio') {
								// 处理单选按钮
								element.filter('[value="' + lastRecord[key] + '"]').prop('checked', true);
							} else if (element.attr('type') === 'checkbox') {
								// 处理复选框
								element.prop('checked', lastRecord[key] === 1 || lastRecord[key] === '1');
							} else {
								// 处理普通输入框
								element.val(lastRecord[key]);
							}
						}
						
						// 处理下拉选择框
						var selectSelector = 'select[name="info[' + key + ']"]';
						var selectElement = $(selectSelector);
						if (selectElement.length > 0) {
							selectElement.val(lastRecord[key]);
						}
					}
				}
				
				// 处理规格数据
				if (lastRecord.guigedata) {
					// 如果有规格数据，调用现有的处理逻辑
					var data = { guigedata: lastRecord.guigedata };
					var guigedata = data.guigedata;
					if(guigedata){
						var gghtml = '<thead><tr><th>规格分组</th><th>规格名称</th><th>操作</th></tr></thead>';
						for(var i=0;i<guigedata.length;i++){
	...
	function saveCurrentRecord() {
		...
		$('input[name^="info["]').each(function() {
			var name = $(this).attr('name').replace('info[', '').replace(']', '');
		...
		// 收集下拉选择框
		$('select[name^="info["]').each(function() {
			var name = $(this).attr('name').replace('info[', '').replace(']', '');
			formData[name] = $(this).val();
		});
		
		// 收集规格数据
		formData.guigedata = window.specs;
		
		// 保存到localStorage
		localStorage.setItem('collage_product_last_record', JSON.stringify(formData));
	}
	...
	$(window).on('beforeunload', function() {
		...
		function refreshgg(){
			...
			$('input[name=ggname]').each(function(){
				...
				window.specs.push({ 'k':i,'title':$(this).val(),'items':ggval});
				i++;
			});
			console.log('refreshgg specs:', window.specs)
			var len = window.specs.length;
			var newlen = 1; 
			var h = new Array(len); 
			...
			var html = '<thead><tr>';
			for(var i=0;i<len;i++){
				html+="<th>" + window.specs[i].title + "</th>";
				var itemlen = window.specs[i].items.length;
				...
				var l = window.specs[i].items.length;
				rowspans[i] = 1;
				for(j=i+1;j<len;j++){
					rowspans[i]*= window.specs[j].items.length;
				}
			}
			html += '<th>规格编号</th>';
			//html += '<th>成本价（元）</th>';
			...
			for(var m=0;m<len;m++){
				...
				for(var j=0;j<newlen;j++){
					...
					if( j % rowspan==0){
						h[m][j]={ k:window.specs[m].items[kid].k,title: window.specs[m].items[kid].title, html: "<td rowspan='" +rowspan + "'>"+ window.specs[m].items[kid].title+"</td>\r\n",id: window.specs[m].items[kid].id};
					}else{
						h[m][j]={ k:window.specs[m].items[kid].k,title:window.specs[m].items[kid].title, html: "",id: window.specs[m].items[kid].id};	
					}
					n++;
					if(n==rowspan){
						kid++; if(kid>window.specs[m].items.length-1) { kid=0; }
						n=0;
					}
				}
			}
			...
			for(var i=0;i<newlen;i++){
				...
				if(typeof(oldgglist[ks])!='undefined'){
					...
					var val = { goods_sn:'',market_price:'',cost_price:'',sell_price:'',weight:'',stock:'1000',pic:'',leader_price:'',leader_commission_rate:''};
					<?php if(getcustom('collage_givescore_time')): if($bid==0): ?>
							var val = { goods_sn:'',market_price:'',cost_price:'',sell_price:'',weight:'',stock:'1000',givescore:0,pic:'',leader_price:'',leader_commission_rate:''};
						<?php endif; ?>
					<?php endif; ?>
				}
			
				hh += '<td>';
				hh += '	<input name="option['+ks+'][goods_sn]" class="layui-input" type="text" style="width:100px" value="'+(val.goods_sn==null?'':(val.goods_sn || val.procode || ''))+'"/>';
				if(i==0){
					hh += ' <i class="fa fa-hand-o-down" style="cursor:pointer" title="批量设置" onclick="plset(\'goods_sn\')"></i>';
				}
				hh += '</td>';
				//hh += '<td>';
				...
				hh += '</td>';
				hh += '<td>';
				hh += ' <input name="option['+ks+'][leader_price]" type="text" style="width:60px" value="'+(val.leader_price==null?'':val.leader_price)+'" class="layui-input"/>';
				if(i==0) {
					hh += ' <i class="fa fa-hand-o-down" style="cursor:pointer" title="批量设置" onclick="plset(\'leader_price\')"></i>';
				}
				hh += '</td>';
				hh += '<td>';
				hh += ' <input name="option['+ks+'][leader_commission_rate]" type="text" style="width:60px" value="'+(val.leader_commission_rate==null?'':val.leader_commission_rate)+'" class="layui-input"/>';
				if(i==0) {
					hh += ' <i class="fa fa-hand-o-down" style="cursor:pointer" title="批量设置" onclick="plset(\'leader_commission_rate\')"></i>';
				}
				hh += '</td>';
				hh += '<td>';
			...
			html+=hh;
			$("#ggvaldiv").html(html);
			layui.form.render('input'); // 重新渲染表单，确保动态生成的输入框被正确处理
		}
		function plset(name){
			if(name =='goods_sn'){
				var goods_sn = $("input[name$='["+name+"]']").eq(0).val();
				if(goods_sn == '') goods_sn = randomString(6);
				$("input[name$='["+name+"]']").each(function(i,v){
					if(i>0) $(this).val(goods_sn+ '-'+i);
				})
			}else{
	...
	layui.form.on('submit(formsubmit)', function(obj){
		try {
			...
			// 验证名店推广拼团时必须选择门店
			var promote_type = $('input[name="info[promote_type]"]:checked').val();
			console.log('promote_type:', promote_type);
			if(promote_type == '1'){
				var mendian_ids = $(".mendianlisttr").map(function(){ return $(this).find("td:eq(0)").text(); }).get().join(",");
				var mendian_count = $('.mendianlisttr').length;
				console.log('mendian_ids:', mendian_ids, 'mendian_count:', mendian_count);
				if(!mendian_ids || mendian_count == 0){
					layer.msg('选择名店推广拼团时必须选择至少一个门店', {icon: 5});
					return false;
				}
				// 更新隐藏输入框的值，确保提交时包含最新的门店ID
				$('input[name="info[mendian_ids]"]').val(mendian_ids);
			}
			...
			// 使用全局变量 specs
			console.log('window.specs:', window.specs);
			field.specs = JSON.stringify(window.specs || []);
			console.log('specs JSON:', field.specs);
			console.log('完整提交数据:', JSON.stringify(field)); // 打印完整提交数据，方便调试
			
			// 从Angular作用域获取编辑器内容
			try {
				var $scope = angular.element(document.querySelector('[ng-controller=FoxController]')).scope();
				var detail_data = JSON.stringify($scope.Items);
				field['info[detail]'] = detail_data;
				console.log('detail data:', detail_data);
			} catch (e) {
				console.error('获取编辑器内容失败:', e);
				// 使用默认值或跳过，避免表单提交失败
				field['info[detail]'] = '[]';
				console.log('使用默认值作为编辑器内容');
			}
			
			console.log('准备提交数据前的完整field:', field);
			var index = layer.load();
			var save_url = "<?php echo url('save'); ?>";
			console.log('请求URL:', save_url);
			
			// 检查jQuery是否可用
			if(typeof $ === 'undefined' || typeof $.post !== 'function') {
				console.error('jQuery或$.post不可用');
				layer.close(index);
				layer.msg('提交功能异常，请刷新页面重试', {icon: 5});
				return false;
			}
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (data.status == 1) {
							...
							layer.msg(data.msg || '提交失败，请重试', {icon: 5});
						}
					},
					error: function(xhr, status, error) {
						console.error('AJAX请求失败:', status, error);
						console.error('xhr状态:', xhr.status);
						console.error('xhr状态文本:', xhr.statusText);
						// 尝试解析响应文本，看是否包含错误信息
						var responseText = xhr.responseText;
						console.error('xhr响应文本:', responseText);
						layer.close(index);
						
						// 尝试从响应文本中提取错误信息
						var errorMsg = error + '，状态码：' + xhr.status;
						if (responseText) {
							// 检查是否是JSON格式
							try {
								...
								if (responseText.indexOf('<') !== -1) {
									...
									var errorElement = tempDiv.querySelector('.err_text');
									if (errorElement) {
										errorMsg = errorElement.textContent || errorMsg;
									}
								} else {
									// 纯文本错误信息
						...
						layer.msg('提交失败：' + errorMsg, {icon: 5});
					}
				});
			} catch (ajax_error) {
				console.error('发送请求时异常:', ajax_error);
				layer.close(index);
				layer.msg('提交异常：' + ajax_error.message, {icon: 5});
			}
		} catch (error) {
			console.error('表单提交异常:', error);
			layer.msg('提交异常：' + error.message, {icon: 5});
		}
		return false; // 阻止默认提交，使用ajax提交
	})
  </script>
  <script>
