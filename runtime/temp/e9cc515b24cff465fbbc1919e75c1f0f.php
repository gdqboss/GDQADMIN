<?php /*a:3:{s:64:"/www/wwwroot/gdqshop.cn/app/view/shop_product/chooseproduct.html";i:1747926690;s:48:"/www/wwwroot/gdqshop.cn/app/view/public/css.html";i:1747926690;s:47:"/www/wwwroot/gdqshop.cn/app/view/public/js.html";i:1747926690;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
						...
						<div class="layui-form layui-col-md12 layui-form-search">
							...
							<div class="layui-inline">
								...
								<div class="layui-input-inline">
									<select name="bid" lay-filter="bidtab">
										<?php if(input('param.showallbusiness')): ?>
										<option value="all">全部</option>
										<?php endif; ?>
										<option value="">自营</option>
										<?php foreach($blist as $b): ?><option value="<?php echo $b['id']; ?>"><?php echo $b['name']; ?></option><?php endforeach; ?>
									</select>
								</div>
							</div>
								<?php if(input('param.showallbusiness')): ?>
									<div id="bnameset" class="layui-inline" >
										<label class="layui-form-label">商户名称</label>
										<div class="layui-input-inline">
											<input type="text" id="bname" name="bname" autocomplete="off" class="layui-input">
										</div>
									</div>
								<?php endif; ?>
							<?php endif; ?>
							...
							<div class="layui-inline">
								...
								<div class="layui-input-inline">
									<select name="gid">
										<option value="">全部</option>
										<?php foreach($glist as $g): ?><option value="<?php echo $g['id']; ?>"><?php echo $g['name']; ?></option><?php endforeach; ?>
									</select>
								</div>
							</div>
							<div class="layui-inline">
...
<script>
  ...
  var table = layui.table;
	var datawhere = {};
	<?php if(input('param.wxvideo_status')): ?>
	datawhere['wxvideo_status'] = '<?php echo app('request')->param('wxvideo_status'); ?>';
	<?php endif; if(input('param.showallbusiness')): ?>
	datawhere['showallbusiness'] = '<?php echo app('request')->param('showallbusiness'); ?>';
	<?php endif; ?>
  //数据表
  var tableIns = table.render({
    elem: '#tabledata'
    ,url: "<?php echo url('index'); if(input('param.wxvideo_status')): ?>&wxvideo_status=<?php echo app('request')->param('wxvideo_status'); ?><?php endif; if(input('param.showallbusiness')): ?>&showallbusiness=<?php echo app('request')->param('showallbusiness'); ?><?php endif; ?>" //数据接口
    ,page: true //开启分页
    ,cols: [[ //表头
	  
      {field: 'id', title: 'ID',  sort: true,width:80},
      <?php if(input('param.showallbusiness')): ?>
      {field: 'bname', title: '所属商户'},
			<?php endif; ?>
      {field: 'name', title: '商品信息',width:320,templet:function(d){
				...
				html += '	<div style="width:100%;">'+(d.cid!=0?'<span style="color:red">['+d.cname+']</span>':'')+d.name+'</div>';
				html += '	<div style="padding-top:5px;color:#f60">￥'+d.sell_price+'</div></div>';
				html += '</div>';
				return html;
			}},
	...
	layui.form.on('select(bidtab)', function(data){
		console.log(data)
		if(data.value == 'all'){
			$('#bnameset').show();
		}else{
	...
	function choosepro(id){
		var mid = "<?php echo app('request')->param('mid'); ?>";
		var index = layer.load();
		$.post("<?php echo url('getproduct'); ?>",{proid:id,mid:mid},function(res){
			layer.close(index);
			if('<?php echo app('request')->param('type'); ?>'=='2'){
				window.parent.choosepro2(res,"<?php echo app('request')->param('args'); ?>");
			}else{
				window.parent.choosepro(res,"<?php echo app('request')->param('args'); ?>");
			}
	...
	function multiselect(){
		...
		var index = layer.load();
		$.post("<?php echo url('getproductlist'); ?>",{proids:ids},function(res){
			layer.close(index);
			window.parent.choosepromulti(res,"<?php echo app('request')->param('args'); ?>");
			var index = parent.layer.getFrameIndex(window.name);
			parent.layer.close(index);
		});
	}
