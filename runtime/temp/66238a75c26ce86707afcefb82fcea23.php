<?php /*a:4:{s:54:"/www/wwwroot/gdqshop.cn/app/view/user_group/index.html";i:1747926690;s:48:"/www/wwwroot/gdqshop.cn/app/view/public/css.html";i:1747926690;s:47:"/www/wwwroot/gdqshop.cn/app/view/public/js.html";i:1747926690;s:54:"/www/wwwroot/gdqshop.cn/app/view/public/copyright.html";i:1747926690;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  ...
  <div class="layui-fluid">
    <div class="layui-row layui-col-space15">
        <div class="layui-card layui-col-md12">
          <div class="layui-card-header"><i class="fa fa-list"></i> 权限组管理
	          <?php if(!$type): ?><i class="layui-icon layui-icon-close" style="font-size:18px;font-weight:bold;cursor:pointer" onclick="closeself()"></i><?php endif; ?>
	        </div>
          <div class="layui-card-body" pad15>
						<div class="layui-col-md6" style="padding-bottom:10px">
							<button class="layui-btn  layuiadmin-btn-list" onclick="openmax('<?php echo url('edit'); ?>/type/<?php echo $type; ?>')">新增</button>
						</div>
						<div class="layui-form layui-col-md6 layui-form-search">
							<div class="layui-inline layuiadmin-input-useradmin">
								<label class="layui-form-label">名称</label>
								<div class="layui-input-inline">
									<input type="text" name="name" autocomplete="off" class="layui-input" value="<?php echo input('get.name'); ?>">
								</div>
							</div>
...
<script>
  ...
  var tableIns = table.render({
    elem: '#tabledata'
    ,url: "<?php echo app('request')->url(); ?>/type/<?php echo $type; ?>" //数据接口
    ,page: true //开启分页
    ,cols: [[ //表头
			{type:"checkbox"},
      {field: 'id', title: 'ID',width:80},
      {field: 'name', title: '名称'},
      <?php if(!$type): ?>{field: 'sort', title: '权重'},<?php endif; ?>
      {field: 'createtime', title: '创建时间',templet:function(d){ return date('Y-m-d H:i',d.createtime)}},
      {field: 'remark', title: '备注'},
      {field: 'operation', title: '操作',templet:function(d){
				var html = '';
				html += '<button class="table-btn" onclick="openmax(\'<?php echo url('edit'); ?>/id/'+d.id+'/type/<?php echo $type; ?>'+'\')">编辑</button>';
				html += '<button class="table-btn" onclick="datadel('+d.id+',<?php echo $type; ?>'+')">删除</button>';
				return html;
      },width:150}
    ]]
  });
	...
	function datadel(id,type=0){
		...
		layer.confirm('确定要删除吗？删除后无法恢复！',{icon: 7, title:'操作确认'}, function(index){
			...
			layer.close(index);
			var index = layer.load();
			$.post("<?php echo url('del'); ?>",{op:'del',id:id,type:type},function(data){
				layer.close(index);
