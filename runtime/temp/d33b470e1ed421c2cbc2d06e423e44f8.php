<?php /*a:4:{s:57:"/www/wwwroot/gdqshop.cn/app/view/mendian_order/index.html";i:1747926690;s:48:"/www/wwwroot/gdqshop.cn/app/view/public/css.html";i:1747926690;s:47:"/www/wwwroot/gdqshop.cn/app/view/public/js.html";i:1747926690;s:54:"/www/wwwroot/gdqshop.cn/app/view/public/copyright.html";i:1747926690;}*/ ?>
        ...
        <div class="layui-card layui-col-md12">
          ...
          <div class="layui-card-body" pad15>
						<div class="layui-col-md12" style="padding-bottom:10px">
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" data-form-export="<?php echo url('excel'); ?>/isdygroupbuy/<?php echo (app('request')->param('isdygroupbuy') ?: 0); ?>/is_free/<?php echo (app('request')->param('is_free') ?: 0); ?>/showtype/<?php echo app('request')->param('showtype'); ?>">导出</button>
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" onclick="plfh2()">发货</button>
							<button class="layui-btn layui-btn-primary layuiadmin-btn-list" onclick="plfh()">全部发货</button>
						</div>
						<div class="layui-col-md12 layui-form search-form layui-form-search">
							...
							<div class="layui-inline">
								<label class="layui-form-label"><?php echo t('门店'); ?></label>
								<div class="layui-input-inline">
									<select name="mdid" lay-search>
										<option value="">全部</option>
										<?php foreach($mendians as $gk=>$gv): ?>
										<option value="<?php echo $gk; ?>"><?php echo $gv; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'operation', title: '操作',minWidth:160,templet:function(d){
				var html = '';
				if(d.paytypeid==5 && d.payorder.check_status>=0){
  ...
  <script>
	  ...
	  function refundChange(e)
	  {
		  ...
		  for(var i in refundNum) {
			  ...
			  refundTotalNum += refundNum[i].num;
		  }
		...
		function plfh2(){
			...
			// html+='	</div>';
			html+='	<div class="layui-form-item" style="margin-top:40px;">';
			html+='		<label class="layui-form-label" style="width:60px"><?php echo t("门店"); ?></label>';
			html+='		<div class="layui-input-inline" style="width:180px">';
			html+='			<select id="plfh2_mdid" style="width:100%;height: 38px;line-height: 38px;border:1px solid #e6e6e6;background-color: #fff;border-radius: 2px;">';
			<?php foreach($mendians as $k=>$v): ?>
			...
			var checkLayer = layer.open({type:1,area:['500px','350px'],title:false,content:html,shadeClose:true,btn: ['选中项发货','全部发货', '取消'],
				yes:function(){
					if(ids.length === 0){
						 return layer.msg('请选择数据');
					}
					var index = layer.load();
					$.post("<?php echo url('plfh2'); ?>",{type:1,ids:ids,express_com:$('#plfh2_express').val(),express_no:$('#plfh2_express_no').val(),mdid:$('#plfh2_mdid').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
				...
				btn2:function(){
					var index = layer.load();
					$.post("<?php echo url('plfh2'); ?>"+urlEncode(datawhere),{type:2,ids:ids,express_com:$('#plfh2_express').val(),express_no:$('#plfh2_express_no').val(),mdid:$('#plfh2_mdid').val()},function(res){
						layer.close(index);
						dialog(res.msg,res.status);
						layer.close(checkLayer);
						tableIns.reload()
