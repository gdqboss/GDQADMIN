<?php /*a:4:{s:56:"/www/wwwroot/gdqshop.cn/app/view/shop_comment/index.html";i:1747926690;s:48:"/www/wwwroot/gdqshop.cn/app/view/public/css.html";i:1747926690;s:47:"/www/wwwroot/gdqshop.cn/app/view/public/js.html";i:1747926690;s:54:"/www/wwwroot/gdqshop.cn/app/view/public/copyright.html";i:1747926690;}*/ ?>
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<link rel="stylesheet" type="text/css" href="/static/fonts/iconfont.css?v=20201218" media="all">
	...
	.layui-table-view .layui-table td .table-imgbox{width:50px;height:50px;float:left;display: flex;align-items: center;justify-content: center}
	.layui-table-view .layui-table td .table-imgbox img{max-width:50px;max-height:50px;float:left;}
	</style>
</head>
<body>
						...
						<div class="layui-form layui-form-search layui-col-md8">
							<div class="layui-inline layuiadmin-input-useradmin">
								...
								<div class="layui-input-inline">
									<input type="text" name="content" autocomplete="off" class="layui-input" value="">
								</div>
							</div>
...
<script>
  ...
  var tableIns = table.render({
    ...
    ,cols: [[ //表头
      ...
      {field: 'name', title: '商品信息',width:320,templet:function(d){
				var html = '';
				html+='<div class="table-imgbox"><img lay-src="'+d.propic+'" src="/static/admin/layui/css/modules/layer/default/loading-2.gif"/></div>';
				html+='<div style="float: left;width:200px;margin-left: 10px;white-space:normal;line-height:20px;">';
				html+='<div style="width:100%;padding-top:5px">'+d.proname+'</div><div style="padding-top:5px;color:#f60">'+d.ggname+'</div>';
				html+='</div>';
				return html;
			}},
      {field: 'nickname', title: '评价人信息',templet: '<div><img src="{{d.headimg}}" style="max-width:50px;"/> {{d.nickname}}</div>',width:150},
      {field: 'score', title: '评分',width:80,sort: true,templet:'<div>{{d.score}}星</div>'},
			{field: 'content', title: '评价内容'},
      {field: 'createtime', title: '评价时间',sort: true,templet:function(d){ return date('Y-m-d H:i',d.createtime)},width:150},
      ...
      {field: 'operation', title: '操作',templet: '<div><button class="table-btn" onclick="showdetail({{d.id}})">详情</button><button class="table-btn" onclick="toreply({{d.id}})">回复</button><button class="table-btn" onclick="datadel({{d.id}})">删除</button></div>',width:200}
    ]],
		// 监听表格的数据更新
  ...
  layui.laydate.render({
	...
	function showdetail(id){
		var index = layer.load();
		$.post("<?php echo url('getdetail'); ?>",{id:id},function(data){
			layer.close(index);
			var html = '';
			layui.laytpl(detailtpl.innerHTML).render(data, function(html){
				detailLayer = layer.open({type:1,area:'500px',title:'评价详情',content:html,resize:true,shadeClose:true});
			});
		})
	}
	function toreply(id){
		layui.laytpl(replytpl.innerHTML).render({}, function(html){
				var toreplyLayer = layer.open({type:1,title:'回复评价',content:html,shadeClose:true,btn:['确定','取消'],yes:function(){
					var index = layer.load();
...
<div class="commentinfo">
	...
	<div class="item">
		<span class="t1">评价时间</span>
		<span class="t2">{{date('Y-m-d H:i',d.detail.createtime)}}</span>
	</div>
	<div class="item">
		<span class="t1">内容</span>
		<span class="t2">{{d.detail.content}}</span>
	</div>
	{{# if(d.detail.content_pic){ }}
	<div class="item">
		<span class="t1">图片</span>
		<span class="t2">
		{{#  $.each(d.detail.content_pic,function(index,item){ }}
			<img src="{{item}}" style="width:50px;max-height:50px;overflow:hidden;margin:4px;cursor:pointer" onclick="previewImg('{{item}}')"/>
		{{# });}}
		</span>
	</div>
	{{# } }}
	{{# if(d.detail.reply_content){ }}
	<div class="item">
		<span class="t1">回复内容</span>
		<span class="t2">{{d.detail.reply_content}}</span>
	</div>
	{{# } }}
</div>
</script>
