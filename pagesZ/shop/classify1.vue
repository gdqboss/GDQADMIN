...
<view class="nav_right">
    <view class="nav_right-content">
        ...
        <view class="classify-ul" v-if="curIndex>-1 && curIndex2>-1 && clist[curIndex].child[curIndex2] && clist[curIndex].child[curIndex2].child && clist[curIndex].child[curIndex2].child.length>0">
            <view class="flex" style="width:100%;overflow-y:hidden;overflow-x:scroll;">
                <block v-for="(item3, idx3) in clist[curIndex].child[curIndex2].child" :key="idx3">
                    <view class="classify-li" :style="curIndex3==idx3?'color:'+t('color1')+';background:rgba('+t('color1rgb')+',0.2)':''" @tap="changeC3Tab" :data-id="item3.id" :data-index="idx3">{{item3.name}}</view>
                </block>
            </view>
        </view>
					<scroll-view class="classify-box" scroll-y="true" @scrolltolower="scrolltolower">
                        <view class="product-itemlist">
                            <view class="item" v-for="(item,index) in datalist" :key="item.id" :class="item.stock <= 0 ? 'soldout' : ''" @tap="goto" :data-url="'/pages/shop/product?id='+item.id">
								<view class="product-pic">
                                <image class="image" :src="item.pic" mode="widthFix" @error="onImgError" :data-index="index"/>
								...
								<view class="product-info">
                                    ...
                                    <view class="p4" v-if="!item.price_type && item.hide_cart!=true" :style="{background:'rgba('+t('color1rgb')+',0.1)',color:t('color1')}" @tap.stop="buydialogChange" :data-proid="item.id"><text class="iconfont icon_gouwuche"></text></view>
								</view>
							</view>
						</view>
    ...
    var locationCache =  app.getLocationCache();
		...
		onPullDownRefresh: function () {
			...
			app.get('ApiShop/classify', {cid:nowcid,bid:that.bid}, function (res) {
			  ...
			  var clist = res.data;
				var ghIndex = -1;
				var targetCID = parseInt(nowcid||0);
				for (var i = 0; i < clist.length; i++) {
					if (targetCID>0){ if (clist[i] && clist[i].id == targetCID) { ghIndex = i; break; } }
					else { if (clist[i] && clist[i].name && clist[i].name.indexOf('港货超市')>-1) { ghIndex = i; break; } }
				}
				if (ghIndex > -1) {
					var children = clist[ghIndex].child || [];
					var newClist = [];
					for (var j = 0; j < children.length; j++) {
						var c = children[j];
						newClist.push({ id: c.id, name: c.name, child: (c.child || []) });
					}
					clist = newClist;
				}
			  that.clist = clist;
			  ...
			  if (nowcid) {
			    for (var i2 = 0; i2 < clist.length; i2++) {
			      if (clist[i2]['id'] == nowcid) {
			        that.curIndex = i2;
			        that.curCid = nowcid;
			      }
			      var downcdata = clist[i2]['child'] || [];
			      var isget = 0;
			      for (var j2 = 0; j2 < downcdata.length; j2++) {
			        if (downcdata[j2]['id'] == nowcid) {
			          that.curIndex = i2;
			          that.curIndex2 = j2;
			          that.curCid = nowcid;
			          isget = 1;
        ...
        onImgError:function(e){
            ...
            if(this.datalist && this.datalist[idx]){
                this.$set(this.datalist[idx],'pic', this.pre_url + '/static/img/nopic.png');
            }
        },
...
.product-itemlist .product-info .p2 .t2 {margin-left:10rpx;font-size:24rpx;color: #aaa;text-decoration: line-through;}
