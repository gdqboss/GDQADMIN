...
<view class="container">
	<block v-if="isload">
		...
		<view class="content-container">
            <view class="nav_left">
                <block v-for="(item, index) in clist" :key="index" v-if="!(item.gid !== undefined && item.gid !== null && item.gid !== '' && (Number(item.gid) === 5 || item.gid === '5' || String(item.gid) === '5' || item.gid.toString() === '5'))">
				...
				<view class="nav_right-content">
                    ...
                    <view class="classify-ul" v-if="curIndex>-1 && clist[curIndex].child.length>0">
                        <view class="flex" style="width:100%;overflow-y:hidden;overflow-x:scroll;">
                         <block v-for="(item, idx2) in clist[curIndex].child" :key="idx2" v-if="!(item.gid !== undefined && item.gid !== null && item.gid !== '' && (Number(item.gid) === 5 || item.gid === '5' || String(item.gid) === '5' || item.gid.toString() === '5'))">
					...
					<scroll-view class="classify-box" scroll-y="true" @scrolltolower="scrolltolower">
						<view class="product-itemlist">
							<view class="item" v-for="(item,index) in datalist" :key="item.id" v-if="!(item.gid !== undefined && item.gid !== null && item.gid !== '' && (Number(item.gid) === 5 || item.gid === '5' || String(item.gid) === '5' || item.gid.toString() === '5'))" :class="item.stock <= 0 ? 'soldout' : ''" @click="goto" :data-url="'/pages/shop/product?id='+item.id">
...
export default {
	...
	onShow:function() {
			...
			console.log('classify.vue 页面显示，强制刷新数据');
			// 完全重置页面状态，确保不使用任何缓存数据
			this.resetPage();
			// 禁用页面缓存，确保每次都重新加载
			// 1. 清除所有相关缓存
			try {
				uni.removeStorageSync('ApiShop/classify');
				uni.removeStorageSync('ApiShop/getprolist');
				uni.removeStorageSync('classify_data');
				uni.removeStorageSync('prolist_data');
				// 清除所有以ApiShop开头的缓存
				const keys = uni.getStorageInfoSync().keys;
				for (let key of keys) {
					if (key.startsWith('ApiShop/')) {
  ...
  onLoad: function (opt) {
	...
	methods: {
		...
		getdata:function(){
			...
			that.datalist = [];
			that.loading = true;
			console.log('开始获取分类数据，直接从数据库读取');
			// 添加唯一时间戳和随机参数，确保请求不被缓存
			var timestamp = new Date().getTime() + Math.random().toString(36).substring(2, 10);
			...
			app.get('ApiShop/classify', {
				...
				that.isload = true;
		      var clist = res.data || [];
		      console.log('原始分类数据:', clist);
		      var filtered = [];
		      for (var i0 = 0; i0 < clist.length; i0++) {
		        var it0 = clist[i0];
		        // 过滤掉"港货超市"分类
		        if (it0 && it0.name && it0.name.indexOf('港货超市') > -1) {
		          console.log('过滤掉港货超市分类:', it0);
		          continue;
		        }
		        // 过滤掉gid=5的分类
		        var gid = it0.gid;
		        ...
		        if (gid !== undefined && gid !== null && gid !== '') {
		          // 检查gid是否为5，无论是什么类型
		          isGid5 = (Number(gid) === 5 || gid === '5' || String(gid) === '5' || gid.toString() === '5');
		          console.log('检查分类gid:', it0.name, it0.id, gid, '是否为5:', isGid5);
		        }
		        if (isGid5) {
		          console.log('过滤掉gid=5的分类:', it0);
		          continue;
		        }
		        // 过滤掉子分类中的gid=5的分类
		        if (it0 && it0.child && it0.child.length > 0) {
		          var child2 = [];
		          for (var j0 = 0; j0 < it0.child.length; j0++) {
		            var c0 = it0.child[j0];
		            // 过滤掉gid=5的子分类
		            var cGid = c0.gid;
		            var isCGid5 = false;
		            // 检查gid是否存在且为5
		            if (cGid !== undefined && cGid !== null && cGid !== '') {
		              // 检查gid是否为5，无论是什么类型
		              isCGid5 = (Number(cGid) === 5 || cGid === '5' || String(cGid) === '5' || cGid.toString() === '5');
		              console.log('检查子分类gid:', c0.name, c0.id, cGid, '是否为5:', isCGid5);
		            }
		            if (isCGid5) {
		              console.log('过滤掉gid=5的子分类:', c0);
		              continue;
		            }
		            child2.push(c0);
		          }
		          it0.child = child2;
		          console.log('分类', it0.name, '的子分类过滤后数量:', child2.length);
		        }
		        filtered.push(it0);
		      }
		      clist = filtered;
		      console.log('过滤后的分类数据:', clist);
		      that.clist = clist;
		      if (!nowcid && clist && clist.length > 0) {
		        that.curIndex = 0;
		        that.curCid = clist[0].id;
		      }
		      if (nowcid) {
		...
		getdatalist: function (loadmore) {
			...
			wherefield.rand = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
			// 强制刷新，不使用任何缓存
			wherefield.forceRefresh = 1;
			// 确保请求不被缓存的额外参数
			wherefield.noCache = 1;
			wherefield._r = Math.random();
			console.log('请求商品列表参数:', wherefield);
			app.post('ApiShop/getprolist', wherefield, function (res) { 
				...
				for(var i = 0; i < data.length; i++){
					...
					if(typeof item === 'object' && item !== null){
						...
						if (gid !== undefined && gid !== null && gid !== '') {
							// 检查gid是否为5，无论是什么类型
							isGid5 = (Number(gid) === 5 || gid === '5' || String(gid) === '5' || gid.toString() === '5');
							console.log('检查商品gid:', item.id, gid, '是否为5:', isGid5);
						}
						
						// 检查是否包含会员等级>1的商品
						var hasHighVipLevel = false;
						if (item.lvprice === 1 && item.lvprice_data) {
							try {
								var lvpriceData = JSON.parse(item.lvprice_data);
								// 检查lvprice_data中是否包含等级>1的会员
								for (var level in lvpriceData) {
									if (Number(level) > 1) {
										hasHighVipLevel = true;
										console.log('商品', item.id, '包含会员等级>', level, '，将被过滤');
										break;
									}
								}
							} catch (e) {
								console.error('解析lvprice_data失败:', e);
							}
						}
						
						// 检查priceshows中是否包含会员等级>1的商品
						if (!hasHighVipLevel && item.priceshows && Array.isArray(item.priceshows)) {
							for (var j = 0; j < item.priceshows.length; j++) {
								var priceShow = item.priceshows[j];
								if (priceShow.id && Number(priceShow.id) > 1) {
									hasHighVipLevel = true;
									console.log('商品', item.id, 'priceshows包含会员等级>', priceShow.id, '，将被过滤');
									break;
								}
							}
						}
						
						// 只有当gid不是5且不包含高等级会员时，才添加到结果中
						if(!isGid5 && !hasHighVipLevel){
							filteredData.push(item);
						}else{
							console.log('过滤掉商品:', item.id, '原因:', isGid5 ? 'gid=5' : '含会员等级>1');
						}
					}
				}
				console.log('第一次过滤后的商品数据:', filteredData);
				if (filteredData.length == 0) {
					if(pagenum == 1){
						...
						that.nomore = true;
						console.log('没有更多商品数据');
					}
				}
				var datalist = that.datalist;
				var newdata = datalist.concat(filteredData);
				console.log('合并后的商品数据:', newdata);
				// 再次过滤，确保没有漏网之鱼
				newdata = newdata.filter(function(item) {
					...
					if (gid !== undefined && gid !== null && gid !== '') {
						isGid5 = (Number(gid) === 5 || gid === '5' || String(gid) === '5' || gid.toString() === '5');
						console.log('再次检查商品gid:', item.id, gid, '是否为5:', isGid5);
					}
					if(isGid5){
						console.log('过滤掉gid=5的商品:', item);
						return false;
					}
					...
					if (item.lvprice === 1 && item.lvprice_data) {
						try {
							...
							for (var level in lvpriceData) {
								if (Number(level) > 1) {
									hasHighVipLevel = true;
									console.log('再次检查商品', item.id, '包含会员等级>', level, '，将被过滤');
									break;
								}
							}
						} catch (e) {
					...
					if (!hasHighVipLevel && item.priceshows && Array.isArray(item.priceshows)) {
						for (var j = 0; j < item.priceshows.length; j++) {
							...
							if (priceShow.id && Number(priceShow.id) > 1) {
								hasHighVipLevel = true;
								console.log('再次检查商品', item.id, 'priceshows包含会员等级>', priceShow.id, '，将被过滤');
								break;
							}
						}
					}
					...
					if(hasHighVipLevel){
						console.log('过滤掉含会员等级>1的商品:', item);
						return false;
					}
					
					return true;
				});
				console.log('最终商品列表数据:', newdata);
